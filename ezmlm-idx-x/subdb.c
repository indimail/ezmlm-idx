 ...
 */
static void
parsesubdb(const char *plugin_name)
{
	unsigned int    j;
	const char     *port;
...
static int
loadsubdb(const char *filename, const char *plugin_name)
{
	line.len = 0;
 ...
 */
static const char *
fixsubdir(const char *subdir)
{
	unsigned int    dir_len;
	if (subdir != 0) {
		if (subdir[0] == '/') {
			dir_len = str_len(listdir);
			if (str_diffn(subdir, listdir, dir_len) != 0 || (subdir[dir_len] != '/' && subdir[dir_len] != 0))
...
const char     *
checktag(unsigned long msgnum, unsigned long listno, const char *action, const char *seed, const char *hash)
{
	const char     *r = 0;
	if ((r = opensub()) != 0)
		return r;
...
int
issub(const char *subdir, const char *userhost, stralloc *recorded)
{
	const char     *r = 0;
	int             result;
...
const char     *
logmsg(unsigned long num, unsigned long listno, unsigned long subs, int done)
{
	const char     *r = 0;
	if (plugin == 0)
		return 0;
...
unsigned long
putsubs(const char *subdir, unsigned long hash_lo, unsigned long hash_hi, int subwrite(const char *, unsigned int))
{
	const char     *r = 0;
	subdir = fixsubdir(subdir);
	if ((r = opensub()) != 0)
...
void
searchlog(const char *subdir, char *search, int subwrite(const char *, unsigned int))
{
	unsigned char  *cps;
	unsigned char   ch;
...
int
subscribe(const char *subdir, const char *userhost, int flagadd, const char *comment, const char *event, int forcehash)
{
	const char     *r = 0;
...
void
tagmsg(unsigned long msgnum, const char *seed, const char *action, char *hashout, unsigned long bodysize, unsigned long chunk)
{
	const char     *r = 0;
...
initsub(const char *subdbline)
{
	...
	if (subdbline == 0) {
		...
		if (!stralloc_copys(&line, subdbline))
			die_nomem();
		parsesubdb(0);
	}
	if (!stralloc_copys(&path, auto_lib()))
		die_nomem();
	if (!stralloc_cats(&path, "/sub-"))
		die_nomem();
	if (!stralloc_cats(&path, info.plugin))
		die_nomem();
	if (!stralloc_cats(&path, ".so"))
		die_nomem();
	if (!stralloc_0(&path))
		die_nomem();
	if ((handle = dlopen(path.s, RTLD_NOW | RTLD_LOCAL)) == 0)
		/*- FIXME: should use MSG functions */
		strerr_die5x(111, FATAL, "Could not load plugin ", path.s, ": ", dlerror());
	else
	if ((plugin = dlsym(handle, "sub_plugin")) == 0)
		/*- FIXME: should use MSG functions */
		strerr_die5x(111, FATAL, "Plugin ", path.s, " is missing symbols: ", dlerror());
}
