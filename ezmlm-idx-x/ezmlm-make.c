...
const char      USAGE[] = "ezmlm-make: usage: ezmlm-make [-+] [ -a..zA..Z03..9 ] dir dot local host";
...
void
f_put(const char *buf, unsigned int len)
{
...
int
read_line(const char *dpm, stralloc *sa)
{
	int             fdin;
	int             match;
	if (sa->len > 0)
...
read_files(void)
{
	...
	if (read_line("/subdb", &popt[6]) != 0 && read_line("/sql", &line) == 0) {
		if (!stralloc_copyb(&popt[6], "sql:", 4))
			die_nomem();
		if (!stralloc_catb(&popt[6], line.s, line.len))
			die_nomem();
	}
...
int
read_file_config(void)
{
	if (!stralloc_copys(&oldflags, "-ABCDEFGHIJKLMNOPqRSTUVWXYZ"))
		die_nomem();
		...
		for (;;) {
			...
			case 'X':
				if (euid > 0 && !flags['c' - 'a'] && (cfname.len == 0))
					if (!stralloc_copys(&cfname, line.s + 2))
						die_nomem();
				break; /* for safety: ignore if root */
			case 'T':
				if (!stralloc_copys(&dot, line.s + 2))
					die_nomem();
				break;
			case 'L':
				if (!stralloc_copys(&local, line.s + 2))
					die_nomem();
				break;
			case 'H':
				if (!stralloc_copys(&host, line.s + 2))
					die_nomem();
				break;
			case 'C':
				if (!stralloc_copys(&code, line.s + 2))
					die_nomem();
				break;
			case 'F':
				if (!stralloc_copys(&oldflags, line.s + 2))
					die_nomem();
				break;
			case 'D':
				break; /* no reason to check */
			default:
				if (ch == '0' || (ch >= '3' && ch <= '9')) {
					if (usecfg && popt[ch - '0'].len == 0)
						if (!stralloc_copys(&popt[ch - '0'], line.s + 2))
							die_nomem();
				} else
...
open_template(stralloc *fn)
{
	...
	if (S_ISDIR(st.st_mode)) {
		--fn->len;
		if (!stralloc_cats(fn, TXT_EZMLMRC))
			die_nomem();
		if (!stralloc_0(fn))
			die_nomem();
	}
...
main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "+aAbBcC:dDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ0:3:4:5:6:7:8:9:"))
		   != opteof) {
		if (opt == 'v' || opt == 'V')
			strerr_die2x(0, "ezmlm-make version: ", auto_version);
		if (opt == 'C') /* treat this like nl switch to allow override of -c */
			if (!stralloc_copys(&cfname, optarg))
				die_nomem();
		if (opt >= 'a' && opt <= 'z') {
			flags[opt - 'a'] = 3; /* Dominant "set" */
			if (opt == 'e')
				flagforce++; /* two 'e' => ignore 'E' */
		} else
		if (opt >= 'A' && opt <= 'Z')
			flags[opt - 'A'] = 2; /* Dominant "unset" */
		else
		if (opt >= '0' && opt <= '9') {
			if (!stralloc_copys(&popt[opt - '0'], optarg))
				die_nomem();
		} else
		if (opt == '+') {
	...
	if (*argv == 0)
		die_usage();
	if (!stralloc_copys(&dir, *argv++))
		die_nomem();
	if (dir.s[0] != '/')
		die_relative();
	if (byte_chr(dir.s, dir.len, '\'') < dir.len)
		die_quote();
	...
	if ((p = *argv++) != 0) {
		if (!stralloc_copys(&dot, p))
			die_nomem();
		if ((p = *argv++) != 0) {
			if (local.len == 0 || str_diff(local.s, p))
				flagforce = 1;	/* must rewrite if list name changed */
			if (!stralloc_copys(&local, p))
				die_nomem();
			if ((p = *argv++) != 0) {
				if (host.len == 0 || str_diff(host.s, p))
					flagforce = 1;	/* must rewrite if list name changed */
				if (!stralloc_copys(&host, p))
					die_nomem();
				if ((p = *argv++) != 0) {
					if (!stralloc_copys(&code, p))
						die_nomem();
				}
			}
		}
	}
	if (dot.len == 0 || local.len == 0 || host.len == 0)
		die_usage();
	if (dot.s[0] != '/')
		die_relative(); /* force absolute dot */
	...
	if (dot.s[slpos] == '/') {
		if (!stralloc_copyb(&template, dot.s, slpos + 1))
			die_nomem(); /* dot dir */
		slpos += byte_chr(dot.s + slpos, dot.len - slpos, '-');
		if (dot.s[slpos]) {
			slpos++;
			if (slpos < dot.len && (pos = slpos + byte_chr(dot.s + slpos, dot.len - slpos, '-')) < dot.len) {
				if (!stralloc_copyb(&popt[1], dot.s + slpos, pos - slpos))
					die_nomem();
				pos++;
				if (pos < dot.len && (slpos = pos + byte_chr(dot.s + pos, dot.len - pos, '-')) < dot.len)
					if (!stralloc_copyb(&popt[2], dot.s + pos, slpos - pos))
						die_nomem();
			}
		}
	}
	...
	if (flags['c' - 'a'] || (cfname.len > 0)) {
		if (!flags['c' - 'a']) {	/* i.e. there is a cfname specified */
			if (!stralloc_copy(&template, &cfname))
				die_nomem();
		} else
		if (!stralloc_cats(&template, TXT_DOTEZMLMRC))
			die_nomem();
		 ...
		 */
		if (!stralloc_copys(&template, auto_etc()))
			die_nomem();
		if (!stralloc_cats(&template, TXT_DEFAULT))
			die_nomem();
		fdin = open_template(&template);
	}
	...
	if (auto_version[i])
		i++;
	j = 0;
	while (line.s[j] == auto_version[i] && j < line.len && auto_version[i] != '.' && auto_version[i]) {
		i++;
		j++; /* major */
	} /* first minor */
	if (auto_version[i] != '.' || j + 1 >= line.len || auto_version[i + 1] != line.s[j + 1])
	...
	for (;;) {
		...
		if (line.s[0] == '<' && line.s[1] == '/') {	/* tag */
			...
			if (pos < line.len - 2) {
				...
				pos++;
				flagnot = 0;
				while (pos < line.len && ((ch = line.s[pos]) != '/' && line.s[pos + 1] != '>')) {
					if (ch == '^') {
						flagnot = 1;
						pos++;
						continue;
				...
				if (pos < line.len && (line.s[pos] != '/' || line.s[pos + 1] != '>'))
			...
			if (line.s[2] == '+') {	/* mkdir */
				if (!flagdo)
					continue;
				if (!stralloc_copys(&dname, "/"))
					die_nomem();
				if (!stralloc_catb(&dname, line.s + 3, pos - 3))
					die_nomem();
				if (!stralloc_0(&dname))
					die_nomem();
				dcreate(dname.s);
			...
			if (line.s[2] == ':') {	/* ln -s */
				...
				if (!stralloc_copyb(&dname, line.s + slpos, pos - slpos))
					die_nomem();
				if (!stralloc_copyb(&lname, line.s + 3, slpos - 3))
					die_nomem();
				if (!stralloc_0(&dname))
					die_nomem();
				if (!stralloc_0(&lname))
			...
			if (pos > 2) { /* </#ai/> => add to open file */
				if (!stralloc_copyb(&fname, line.s + 1, pos - 1))
					die_nomem();
				if (!stralloc_0(&fname))
					die_nomem();
			}
			...
			if (str_diff(fname.s, oldfname.s)) {
				...
				if (flagdo && flagnotexist) {
					...
					f_open(fname.s);
					if (!stralloc_copy(&oldfname, &fname))
						die_nomem();
				}
			}
			if (flagdo)
		...
		outline.len = 0;
		for (;;) {
			if (next < line.len && (pos = next + byte_chr(line.s + next, line.len - next, '<')) 
					< line.len && line.s[pos + 1] == '#' && line.s[pos + 2] &&
					line.s[pos + 3] == '#' && line.s[pos + 4] == '>') {	/* host/local */
				if (!stralloc_catb(&outline, line.s + last + 1, pos - last - 1))
					die_nomem();
				switch (line.s[pos + 2]) {
				case 'B': /* path to ezmlm binaries (no trailing /) */
					if (!stralloc_cats(&outline, auto_bin()))
						die_nomem();
					last = pos + 4;
					next = pos + 5;
					break;
				case 'C': /* digestcode */
					if (!stralloc_cat(&outline, &code))
						die_nomem();
					last = pos + 4;
					next = pos + 5;
					break;
				case 'D': /* listdir */
					if (!stralloc_cat(&outline, &dir))
						die_nomem();
					last = pos + 4;
					next = pos + 5;
					break;
				case 'F': /* flags */
					if (!stralloc_cat(&outline, &f))
						die_nomem();
					last = pos + 4;
					next = pos + 5;
					break;
				case 'H': /* hostname */
					if (!stralloc_cat(&outline, &host))
						die_nomem();
					last = pos + 4;
					next = pos + 5;
					break;
				case 'L': /* local */
					if (!stralloc_cat(&outline, &local))
						die_nomem();
					last = pos + 4;
					next = pos + 5;
					break;
				case 'T': /* dot */
					if (!stralloc_cat(&outline, &dot))
						die_nomem();
					last = pos + 4;
					next = pos + 5;
					break;
				case 'X': /* config file name */
					if (!stralloc_cat(&outline, &cfname))
						die_nomem();
					last = pos + 4;
					next = pos + 5;
					break;
				default: /* copy unknown tag as is for e.g. <#A#> and */
					...
					if (!stralloc_catb(&outline, line.s + pos, 5))
						die_nomem();
					last = pos + 4;
					next = pos + 5;
					break;
