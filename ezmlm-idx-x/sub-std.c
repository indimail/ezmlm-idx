...
static const char *
_logmsg(struct subdbinfo *info, unsigned long msgnum, unsigned long listno, unsigned long subs, int done)
{
	return 0;
	(void) info;
	(void) msgnum;
	...
	for (i = hash_lo; i <= hash_hi; ++i) {
		...
		if (fd == -1) {
			...
			for (;;) {
				...
				if (!match)
					break;
				if (subwrite(line.s + 1, line.len - 2) == -1)
					die_write("stdout");
				no++;
			}
			close(fd);
...
lineout(const stralloc *line, int subwrite(const char *, unsigned int))
{
	...
	if (!stralloc_cats(&outline, ": "))
		die_nomem();
	if (!stralloc_catb(&outline, line->s, line->len - 1))
		die_nomem();
 ...
 */
static int
_subscribe(struct subdbinfo *info, const char *subdir, const char *userhost, int flagadd, const char *comment, const char *event,
		   int forcehash)
{
	static stralloc line = { 0 };
	...
	if ((j = byte_rchr(addr.s, addr.len, '@')) == addr.len)
	...
	if (!stralloc_copyb(&fnnew, fn.s, fn.len - 1))
		die_nomem();
...
static void
_tagmsg(struct subdbinfo *info, unsigned long msgnum, const char *hashout, unsigned long bodysize, unsigned long chunk)
{
	(void) info;
	(void) msgnum;
	(void) hashout;
...
static const char *
_mktab(struct subdbinfo *info)
{
	if (mkdir("subscribers", 0777) < 0 && errno != error_exist)
		return error_str(errno);
	if (mkdir("allow/subscribers", 0777) < 0 && errno != error_exist)
		return error_str(errno);
	if (mkdir("deny/subscribers", 0777) < 0 && errno != error_exist)
		return error_str(errno);
	if (mkdir("digest/subscribers", 0777) < 0 && errno != error_exist)
		return error_str(errno);
	if (mkdir("mod/subscribers", 0777) < 0 && errno != error_exist)
		return error_str(errno);
	return 0;
	(void) info;
}
...
rmsubs(const char *subdir)
{
	...
	if (rmdir(fn.s) < 0 && errno != error_noent)
		return error_str(errno);
	return 0;
}

static const char *
_rmtab(struct subdbinfo *info)
{
	const char     *r;
	if ((r = rmsubs(".")) != 0)
		return r;
