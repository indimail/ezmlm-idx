/*
 * $Id: ezmlm-gate.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
const char      USAGE[] = "ezmlm-gate: usage: ezmlm-gate [-cCmMpPqrRsSvV] dir [moddir [...]]";
...
main(int argc, char **argv)
{
	 ...
	 */
	if (!stralloc_catb(&storeopt, sendopt.s + 1, sendopt.len - 1))
		die_nomem();
	...
	if (queryext) {
		...
		for (j = 0; j < cmds.len; ++j)
			if (!cmds.s[j]) {
				...
				case '\0':
				case '#':
					break; /* ignore blank/comment */
				case '|':
					ret = mailprog(cmds.s + i + 1);
					break;
				default:
					ret = mailprog(cmds.s + i);
					break;
				}
				if (ret)
					break;
