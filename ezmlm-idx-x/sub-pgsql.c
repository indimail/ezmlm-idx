...
static void
dummyNoticeProcessor(void *arg, const char *message)
{
	(void) arg;
	(void) message;
}
 ...
 */
static const char *
_opensub(struct subdbinfo *info)
{
	...
	if (info->conn == 0) {
		...
		info->conn = PQsetdbLogin(info->host, info->port ? strnum : "", NULL, NULL, info->db, info->user, info->pw);
		/*
 ...
 */
static void
_closesub(struct subdbinfo *info)
{
	if ((PGconn *) info->conn)
...
static void
die_sqlerror(struct subdbinfo *info)
{
	strerr_die2x(111, FATAL, PQerrorMessage((PGconn *) info->conn));
}

static void
die_sqlresulterror(PGresult *result)
{
	strerr_die2x(111, FATAL, PQresultErrorMessage(result));
}

PGresult       *
_execute(struct subdbinfo *info, struct stralloc *q, unsigned int nparams, struct stralloc *params)
{
	PGresult       *result;
...
int
sql_exec(struct subdbinfo *info, struct stralloc *q, unsigned int nparams, struct stralloc *params)
{
	PGresult       *result;
	unsigned long   rows;
	const char     *err;
...
void           *
sql_select(struct subdbinfo *info, struct stralloc *q, unsigned int nparams, struct stralloc *params)
{
	PGresult       *result;
	struct _result *ret;
...
int
sql_fetch_row(struct subdbinfo *info, void *result, unsigned int ncolumns, struct stralloc *columns)
{
...
extern void
sql_free_result(struct subdbinfo *info, void *result)
{
	struct _result *r = result;
	PQclear(r->res);
	free(r);
...
int
sql_table_exists(struct subdbinfo *info, const char *name)
{
	PGresult       *result;
	 ...
	 */
	if (!stralloc_copys(&line, "SELECT 0 FROM "))
		return -1;
	if (!stralloc_cats(&line, name))
		return -1;
	if (!stralloc_cats(&line, " LIMIT 1"))
		return -1;
	if (!stralloc_0(&line))
		return -1;
...
const char     *
sql_create_table(struct subdbinfo *info, const char *definition)
{
	PGresult       *result;
 ...
 * * as a primary key. 
 */
const char      sql_sub_table_defn[] = "  hash    INT4 NOT NULL," "  address VARCHAR(255) NOT NULL PRIMARY KEY";
...
const char      sql_slog_table_defn[] =
	"  tai	TIMESTAMP DEFAULT now()," "  address	VARCHAR(255) NOT NULL," "  fromline	VARCHAR(255) NOT NULL,"
	"  edir	CHAR NOT NULL," "  etype	CHAR NOT NULL";

/*
 * main list inserts a cookie here. Sublists check it 
 */
const char      sql_cookie_table_defn[] =
	"  msgnum	INT4 NOT NULL PRIMARY KEY," "  tai	TIMESTAMP NOT NULL DEFAULT now()," "  cookie	CHAR(20) NOT NULL,"
	"  chunk	INT4 NOT NULL DEFAULT 0," "  bodysize	INT4 NOT NULL DEFAULT 0";
...
const char      sql_mlog_table_defn[] =
	"msgnum	INT4 NOT NULL," "listno	INT4 NOT NULL," "tai		TIMESTAMP DEFAULT now()," "subs		INT4 NOT NULL DEFAULT 0,"
	"done		INT4 NOT NULL DEFAULT 0," "PRIMARY KEY (listno,msgnum,done)";
...
const char     *
sql_drop_table(struct subdbinfo *info, const char *name)
{
	PGresult       *result;

	if (!stralloc_copys(&line, "DROP TABLE "))
		die_nomem();
	if (!stralloc_cats(&line, name))
		die_nomem();
	if (!stralloc_0(&line))
