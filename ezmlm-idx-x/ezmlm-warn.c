/*
 * $Id: ezmlm-warn.c,v 1.1 2025-01-22 10:52:36+05:30 Cprogrammer Exp mbhangui $
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
const char      USAGE[] = "ezmlm-warn: usage: ezmlm-warn -dD -l secs -t days dir";
...
doit(int flagdig, int flagw)
{
	...
	if (qmail_copy(&qq, &ssin, copylines) < 0)
		die_read();
	close(fd);
...
dodir(int flagdig)
{
	...
	if (!stralloc_copys(&fnlastd, workdir))
		die_nomem();
	if (!stralloc_cats(&fnlastd, "/bounce/lastd"))
		die_nomem();
	if (!stralloc_0(&fnlastd))
		die_nomem();
	...
	if (!stralloc_copy(&fnlasth, &fnlastd))
		die_nomem();
	...
	if (!stralloc_copys(&line, workdir))
		die_nomem();
	if (!stralloc_cats(&line, "/bounce/d"))
		die_nomem();
	if (!stralloc_0(&line))
		die_nomem();
	...
	while ((d = readdir(bouncedir))) {	/* dxxx/ */
		...
		if (when >= bouncedate * 10000 + bouncetimeout) {
			if (!stralloc_copys(&bdname, workdir))
				die_nomem();
			if (!stralloc_cats(&bdname, "/bounce/d/"))
				die_nomem();
			if (!stralloc_cats(&bdname, d->d_name))
				die_nomem();
			if (!stralloc_0(&bdname))
				die_nomem();
			...
			while ((ds = readdir(bsdir))) {	/* dxxxx/yyyy */
				if (str_equal(ds->d_name, "."))
					continue;
				if (str_equal(ds->d_name, ".."))
					continue;
				if (!stralloc_copy(&fn, &bdname))
					die_nomem();	/* '\0' at end */
				fn.s[fn.len - 1] = '/';
				if (!stralloc_cats(&fn, ds->d_name))
					die_nomem();
				if (!stralloc_0(&fn))
					die_nomem();
				if ((ds->d_name[0] == 'd') || (ds->d_name[0] == 'w'))
					doit(flagdig, ds->d_name[0] == 'w');
				else /* other stuff is junk */ if (unlink(fn.s) == -1)
					strerr_die2sys(111, FATAL, MSG1(ERR_DELETE, fn.s));
			}
			closedir(bsdir);
	...
	if (when < (unsigned long) st.st_mtime + 100000 && when > (unsigned long) st.st_mtime)
	...
	if (!stralloc_copys(&line, workdir))
		die_nomem();
	if (!stralloc_cats(&line, "/bounce/h/"))
		die_nomem();
	if (!stralloc_catb(&line, lasth.s, 1))
		die_nomem();
	if (!stralloc_0(&line))
		die_nomem();
	...
	if (!hdir) {
		...
		while ((d = readdir(hdir))) {
			...
			if (str_equal(d->d_name, ".."))
				continue;
			if (!stralloc_copys(&fn, line.s))
				die_nomem();
			if (!stralloc_append(&fn, "/"))
				die_nomem();
