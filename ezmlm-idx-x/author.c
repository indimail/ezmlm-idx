/*
 * $Id: author.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include "die.h"
#include "mime.h"

void
author_name(stralloc *out, const char *s, unsigned int l)
/*
 ...
 */
{
	...
	if (cpquot && cpquotlast >= cpquot) {
		cpfirst = cpquot;
		cp = cpquotlast;
	} else
	if (cpcom && cpcomlast >= cpcom) {
		cpfirst = cpcom;
		cp = cpcomlast;
	} else {
		cp = s + l - 1;
	...
	for (;;) { /* e.g. LWSP <user@host> */
		while (cpfirst <= cp && (*cpfirst == ' ' || *cpfirst == '\t' || *cpfirst == '<'))
			cpfirst++;
		while (cp >= cpfirst && (*cp == ' ' || *cp == '\t' || *cp == '\n' || *cp == '>'))
			cp--;
...
void
author_addr(stralloc *out, const char *s, unsigned int l)
/* s is a string that contains a from: line argument\n. We parse
 * s as follows: If there is an unquoted '<' we eliminate everything after
 * it else if there is a unquoted ';' we eliminate everything after it.
 * Then, we eliminate LWSP and '"' from the beginning and end. Note that
 ...
 */
{
	...
	/*-
	 * make cplt point to first character of address,
	 * cpgt point to one character after end 
	 */
	if (cplt && cpgt)
		++cplt;
	else
	if (cpat) {
		/*
		 * backup strategy, we found an @, grab
