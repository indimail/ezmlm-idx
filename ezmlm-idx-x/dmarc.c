/*
 * $Id: dmarc.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include "byte.h"
#include "die.h"
#include "dmarc.h"
...
int
dmarc_fetch(stralloc *out, const char *domain)
{
	if (!stralloc_copys(&ddomain, "_dmarc."))
		die_nomem();
	if (!stralloc_cats(&ddomain, domain))
		die_nomem();
	if (dns_txt(out, &ddomain) < 0)
		return -1;
	/*- Check if it's really a DMARC record */
	if (out->len < 10)
		return 0;
	if (byte_diff(out->s, 9, "v=DMARC1;"))
		return 0;
	return 1;
}

int
dmarc_get(const stralloc *rec, const char *key, stralloc *out)
{
	const char     *rp;
	...
	while (end - rp > keylen + 1) {
		if (byte_equal(rp, keylen, key) && (rp[keylen] == '=' || rp[keylen] == ' ')) {
			...
			while (tp > rp && tp[-1] == ' ')
				--tp;
			/*- Finally, can return */
			if (!stralloc_copyb(out, rp, tp - rp))
				die_nomem();
			return 1;
		}
		while (rp < end && *rp++ != ';');
...
int
dmarc_p_reject(const char *domain)
{
