/*
 * $Id: ezmlm-cron.c,v 1.1 2025-01-22 10:47:13+05:30 Cprogrammer Exp mbhangui $
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <pwd.h>
...
const char      USAGE[] = "ezmlm-cron: usage: ezmlm-cron [-cCdDlLvV] [-w dow] [-t hh:mm] [-i hrs] listadr code";
 ...
 *
 * flagaddr - 1 for addresses with '@', 0 for other args
 */
static int
isclean(char *addr, int flagaddr)
{
	unsigned int    pos;
	char            ch;
	char           *cp;
...
main(int argc, char **argv)
{
	...
	if (!stralloc_copys(&user, ppasswd->pw_name))
		die_nomem();
	if (!stralloc_0(&user))
		die_nomem();
	if (!(ppasswd = getpwuid(euid)))
		strerr_die2x(100, FATAL, MSG(ERR_EUID));
	if (!stralloc_copys(&dir, ppasswd->pw_dir))
		die_nomem();
	if (!stralloc_0(&dir))
		die_nomem();
	if (!stralloc_copys(&euser, ppasswd->pw_name))
		die_nomem();
	if (!stralloc_0(&euser))
		die_nomem();
	...
	if (!local) {
		...
		if (!stralloc_copys(&listaddr, local))
			die_nomem();
		if (!isclean(local, 1))
			die_argument();
		pos = str_chr(local, '@');
 ...
 */
	...
	if (flagconfig) {
		line.s[line.len - 1] = '\n'; /* not very elegant ;-) */
		substdio_fdbuf(&ssout, (ssize_t(*)(int, char *, size_t)) write, 1, outbuf, sizeof (outbuf));
	...
	if (!flaglist) {
		if (!stralloc_copyb(&addr, strnum, fmt_ulong(strnum, mm)))
			die_nomem();
		...
		if (!dh) {
			...
			if (!stralloc_catb(&addr, strnum, fmt_ulong(strnum, hh)))
				die_nomem();
			for (t = hh + dh; t < hh + 24L; t += dh) {
				if (!stralloc_cats(&addr, ","))
					die_nomem();
				if (!stralloc_catb(&addr, strnum, fmt_ulong(strnum, t % 24L)))
					die_nomem();
			}
		}
		if (!stralloc_cats(&addr, " * * "))
			die_nomem();
		if (!stralloc_cats(&addr, dow))
			die_nomem();
		if (!stralloc_cats(&addr, " "))
			die_nomem();
		if (!stralloc_cats(&addr, auto_prefix))
			die_nomem();
		if (!stralloc_cats(&addr, qmail_inject))
			die_nomem();
		if (!stralloc_cats(&addr, local))
			die_nomem();
		if (!stralloc_cats(&addr, "-dig-"))
			die_nomem();
		if (!stralloc_cats(&addr, code))
			die_nomem();
		if (!stralloc_cats(&addr, "@"))
			die_nomem();
		if (!stralloc_cats(&addr, host))
			die_nomem();
		/*- feed 'Return-Path: <user@host>' to qmail-inject */
		if (!stralloc_cats(&addr, "%Return-path: <"))
			die_nomem();
		if (!stralloc_cats(&addr, user.s))
			die_nomem();
		if (!stralloc_cats(&addr, "@"))
			die_nomem();
		if (!stralloc_cat(&addr, &rp))
			die_nomem();
		if (!stralloc_cats(&addr, ">\n"))
			die_nomem();
	}
	...
	if (fdin != -1) {
		for (;;) {
			...
			pos = str_chr(line.s, '/');
			if (!str_start(line.s + pos, auto_prefix))
				continue;
			pos += str_len(auto_prefix);
			if (!str_start(line.s + pos, qmail_inject))
				continue;
			...
			if (!local) {
				...
				if (pos + lenhost + 1 >= line.len)
					continue;
				if (case_diffb(line.s + pos + 1, lenhost, host))
					continue;
				if (line.s[pos + lenhost + 1] != '%')
					continue;
				/*- check local */
				if (poslocal + lenlocal + 5 >= line.len)
					continue;
				if (!str_start(line.s + poslocal, local))
					continue;
				pos2 = poslocal + lenlocal;
				if (!str_start(line.s + pos2, "-dig-"))
					continue;
				foundlocal = 1;
			}
			if (foundlocal) {
	...
	if (!stralloc_copys(&line, auto_cron))
		die_nomem();
	if (!stralloc_cats(&line, "/crontab '"))
		die_nomem();
	if (!stralloc_cats(&line, dir.s))
		die_nomem();
	if (!stralloc_cats(&line, "/crontab'"))
		die_nomem();
	if (!stralloc_0(&line))
...
/*
 * $Log: ezmlm-cron.c,v $
 * Revision 1.1  2025-01-22 10:47:13+05:30  Cprogrammer
 * Fixes for gcc14
 *
 */
