/*
 * $Id: ezmlm-archive.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
const char      USAGE[] = "ezmlm-archive: usage: ezmlm-archive [-cCFsSTvV] [-f min_msg] [-t max_msg] dir";
 ...
 */
void
close_proper(substdio *ss, const char *s, const char *sn)
{
 ...
 */
void
write_threads(const msgentry *msgtable, subentry *subtable,
		const authentry *authtable, const dateentry *datetable,
		unsigned long from, unsigned long to)
{
	...
	while (psubtm->sub) { /* these are in msgnum order */
		...
		if (psubtm->firstmsg >= nextmsg) { /* done with this month */
			...
			if (!stralloc_copys(&fn, "archive/threads/"))
				die_nomem();
			if (!stralloc_catb(&fn, strnum, fmt_uint(strnum, startdate)))
				die_nomem();
			if (!stralloc_copy(&fnn, &fn))
				die_nomem();
			if (!stralloc_0(&fn))
				die_nomem();
			if (!stralloc_cats(&fnn, "n"))
				die_nomem();
			if (!stralloc_0(&fnn))
				die_nomem();
		...
		if (psubtm->firstmsg < nextmsg && psubtm->lastmsg >= startmsg) {
			...
			if (!stralloc_cats(&line, ":"))
				die_nomem();
			if (!stralloc_catb(&line, psubtm->sub, HASHLEN))
				die_nomem();
			if (!stralloc_cats(&line, " ["))
				die_nomem();
			if (!stralloc_catb(&line, strnum, fmt_ulong(strnum, (unsigned long) psubtm->msginthread)))
				die_nomem();
			if (!stralloc_cats(&line, "]"))
				die_nomem();
	...
	while (psubt->sub) { /* now the threads */
		if (!stralloc_copys(&fn, "archive/subjects/"))
			die_nomem();
		if (!stralloc_catb(&fn, psubt->sub, 2))
			die_nomem();
		if (!stralloc_0(&fn))
			die_nomem();
		...
		fn.s[fn.len - 1] = '/';
		if (!stralloc_catb(&fn, psubt->sub + 2, HASHLEN - 2))
			die_nomem();
		if (!stralloc_copy(&fnn, &fn))
			die_nomem();
		if (!stralloc_cats(&fnn, "n"))
		...
		for (msg = psubt->firstmsg; msg <= psubt->lastmsg; msg++) {
			if (pmsgt->subnum == subnum) {
				if (!stralloc_copyb(&line, strnum, fmt_ulong(strnum, msg)))
					die_nomem();
				if (!stralloc_cats(&line, ":"))
					die_nomem();
	...
	while (pautht->auth) { /* now the authors */
		if (!stralloc_copys(&fn, "archive/authors/"))
			die_nomem();
		if (!stralloc_catb(&fn, pautht->auth, 2))
			die_nomem();
		if (!stralloc_0(&fn))
			die_nomem();
		if (mkdir(fn.s, 0755) == -1)
			if (errno != error_exist)
				strerr_die2sys(111, FATAL, MSG1(ERR_CREATE, fn.s));
		fn.s[fn.len - 1] = '/';
		if (!stralloc_catb(&fn, pautht->auth + 2, HASHLEN - 2))
			die_nomem();
		if (!stralloc_copy(&fnn, &fn))
			die_nomem();
		if (!stralloc_cats(&fnn, "n"))
