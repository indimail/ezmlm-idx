/*
 * $Id: hdr_datemsgid.c,v 1.1 2025-01-22 10:52:36+05:30 Cprogrammer Exp mbhangui $
 */
#include <unistd.h>
#include "hdr.h"
#include "qmail.h"
...
void
hdr_datemsgid(unsigned long when)
{
	...
	if (!stralloc_catb(&line, strnum, fmt_ulong(strnum, (unsigned long) getpid())))
		die_nomem();
	if (!stralloc_cats(&line, ".ezmlm@"))
		die_nomem();
	if (!stralloc_cats(&line, outhost.s))
		die_nomem();
	if (!stralloc_0(&line))
		die_nomem();
