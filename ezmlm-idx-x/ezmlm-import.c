/*
 * $Id: ezmlm-import.c,v 1.1 2025-01-22 10:52:38+05:30 Cprogrammer Exp mbhangui $
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
const char      USAGE[] = "ezmlm-import: usage: ezmlm-import dir [/path/to/mbox]";
...
openone(unsigned long outnum)
{
	...
	if (!stralloc_cats(&fnaf, "/"))
		die_nomem();
	if (!stralloc_catb(&fnaf, strnum, fmt_uint0(strnum, (unsigned int) (outnum % 100), 2)))
		die_nomem();
...
static void
numwrite(unsigned long msgnum, unsigned long cumsize)
{
	...
	if ((fd = open_trunc("numnew")) == -1 || write(fd, strnum, i) != i || fsync(fd) == -1 || close(fd) == -1)
...
{
	if (fd > 0)
		if (substdio_flush(&ssarchive) == -1 || fchmod(fd, MODE_ARCHIVE | 0700) == -1 || close(fd) == -1)
			strerr_die2sys(111, FATAL, MSG1(ERR_WRITE, fnaf.s));
}
...
main(int argc, char *argv[])
{
	...
	while (getln(&ssin, &line, &match, '\n') == 0 && match) {
		if (line.len > 5 && byte_diff(line.s, 5, "From ") == 0) {
			flushit(fd);
			++msgnum;
