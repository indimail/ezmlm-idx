/*
 * $Id: idxthread.c,v 1.1 2025-01-22 10:52:36+05:30 Cprogrammer Exp mbhangui $
 *
 ...
 */
static void
newsub(subentry *psubt, const char *subject, unsigned int sublen, unsigned long msg)
{
	const char     *cpfrom;
	char           *cpto;
	unsigned int    cpno;
 ...
 */
static void
newauth(authentry *pautht,		/* entry for current message */
...
init_dummy(void)
{
	...
	if (!stralloc_ready(&dummyind, HASHLEN + 1))
		die_nomem();
	for (i = 0; i < HASHLEN; i++)
		dummyind.s[i] = 'a';
	dummyind.len = HASHLEN;
	if (!stralloc_append(&dummyind, " "))
		die_nomem();
}
 ...
 */
void
idx_mkthreads(msgentry **pmsgtable,	/* table of message<->subject */
			  ...
			  int locked /* if already locked */)
{
	...
	for (idx = msg_from / 100; idx <= idxto; idx++) {
		...
		if (!stralloc_copys(&line, "archive/"))
			die_nomem();
		if (!stralloc_catb(&line, strnum, fmt_ulong(strnum, idx)))
			die_nomem();
		if (!stralloc_cats(&line, "/index"))
			die_nomem();
		if (!stralloc_0(&line))
			die_nomem();
		...
		if (!msg)
			msg = 1L;			/* for start to make msg > tmpmsg */
		...
		for (; msg <= endmsg; msg++) {
			...
			if (hasauth) {
				pos = 0;
				while (pos < authline.len && authline.s[pos] != ' ')
					pos++;
				if (++pos < authline.len) {
 ...
 */
void
idx_mkthread(msgentry **pmsgtable,	/* pointer to table of message<->subject */
			 subentry **psubtable,	/* ptr to tbl of subject no, len, str char * */
			 authentry **pauthtable, unsigned long msg_from,	/* first message in range */
			 unsigned long msg_to,	/* last message in range */
 ...
 */
void
idx_mklist(msgentry **pmsgtable,	/* pointer to table of message<->subject */
		   subentry **psubtable,	/* ptr to tbl of subject no, len, str char * */
		   authentry **pauthtable, unsigned long msg_from,	/* first message in range */
		   unsigned long msg_to /* last message in range */)
{
 ...
 */
void
idx_destroythread(msgentry *msgtable, subentry *subtable, authentry *authtable)
{
	subentry       *psubt;
	authentry      *pautht;
