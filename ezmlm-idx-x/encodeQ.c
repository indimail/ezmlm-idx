/*
 * $Id: encodeQ.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include "messages.h"
#include "mime.h"
#include "stralloc.h"
 ...
 */
void
encodeQ(const char *indata, unsigned int n, stralloc *outdata)
{
	char           *cpout;
	char            ch;
	unsigned int    i;
	...
	if (!stralloc_copys(outdata, ""))
		die_nomem();
	if (!stralloc_ready(outdata, n * 3 + n / 36))
		die_nomem(); /* worst case */
	cpout = outdata->s;
	while (n--) {
		ch = *cpin++;
		if (ch != ' ' && ch != '\n' && ch != '\t' && (ch > 126 || ch < 33 || ch == 61)) {
			*(cpout++) = '=';
