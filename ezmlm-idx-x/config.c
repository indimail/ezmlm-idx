/*
 * $Id: config.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include <unistd.h>
#include "byte.h"
#include "config.h"
...
static void
parse_flags(const char *start, int len)
{
	while (len > 0) {
		const char      ch = *start;
		if (ch >= 'A' && ch <= 'Z')
...
load_config(void)
{
	...
	if (!stralloc_copy(&mainlocal, &outlocal))
		die_nomem();

	getconf_line(&listid, "listid", 0);
	if (getconf_line(&charset, "charset", 0)) {
		if (charset.len >= 2 && charset.s[charset.len - 2] == ':') {
			if (charset.s[charset.len - 1] == 'B' || charset.s[charset.len - 1] == 'Q') {
				flagcd = charset.s[charset.len - 1];
				charset.s[charset.len - 2] = '\0';
			}
		}
	} else
	if (!stralloc_copys(&charset, TXT_DEF_CHARSET))
		die_nomem();
	if (!stralloc_0(&charset))
		die_nomem();
}

void
startup(const char *dir)
{
	if (dir == 0)
		die_usage();
...
static int
getflag(struct flag *flagset, int flagno)
{
...
int
flag_isset(char flag)
{
	return (flag >= 'A' && flag <= 'Z') ? getflag(flags, flag - 'A')
...
int
flag_isnameset(const char *name)
{
	int             i;
	for (i = 0; i < NO_FLAGS; ++i) {
		if (flags[i].filename != 0 && str_diff(name, flags[i].filename) == 0)
			return flags[i].state;
	}
	for (i = 0; i < 10; ++i) {
		if (numflags[i].filename != 0 && str_diff(name, numflags[i].filename) == 0)
