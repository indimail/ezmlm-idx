/*
 * $Id: ezmlm-subunsub.c,v 1.1 2025-01-22 10:52:36+05:30 Cprogrammer Exp mbhangui $
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
void
subunsub_main(int submode, int argc, char **argv)
{
	const char     *subdir;
	const char     *addr;
	const char     *comment;
	...
	if (!stralloc_copys(&event, submode ? "+" : "-"))
		die_nomem();
	if (!stralloc_cats(&event, flagtag))
		die_nomem();
	if (!stralloc_0(&event))
		die_nomem();
