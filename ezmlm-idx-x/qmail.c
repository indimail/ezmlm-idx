...
qmail_open(struct qmail *qq)
{
	...
	case 0:
		...
		etc_dir = auto_etc();
		if (!stralloc_copys(&tmp, etc_dir) || !stralloc_catb(&tmp, "/global_vars", 12) || !stralloc_0(&tmp))
			_exit(51);
		if (!access(tmp.s, X_OK)) {
			...
			_exit(55);
		if (!binqqargs[0])
			binqqargs[0] = env_get("EZMLMQUEUE");
		if (!binqqargs[0])
...
void
qmail_put(struct qmail *qq, const char *s, int len)
{
	if (!qq->flagerr && substdio_put(&qq->ss, s, len) == -1)
		qq->flagerr = 1;
	qq->msgbytes += len;
}
