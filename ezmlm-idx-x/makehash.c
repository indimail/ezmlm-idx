 ...
 */
void
makehash(const char *indata, unsigned int inlen, char *hash)
{
	unsigned char   h[32];
	surfpcs         s;
	uint32          seed[32];
 ...
 */
void
mkauthhash(const char *s, unsigned int len, char *h)
{
	...
	if (i < len) { /* if not then i=sa->len, j=k=l=0 */
		...
		while (l < i && s[l] != '-')
			++l;
		if (!stralloc_copyb(&dummy, s + k, l - k))
			die_nomem();
		if (!stralloc_catb(&dummy, s + i, j - i))
			die_nomem();
