/*
 * $Id: decodeB.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include "stralloc.h"
#include "strerr.h"
#include "uint32.h"
 ...
 */
void
decodeB(const char *cpfrom, unsigned int n, stralloc *outdata)
{
	uint32          hold32;
	char            holdch[4] = "???";
	int             i, j;
	...
	i = 0;
	hold32 = 0L;
	if (!stralloc_readyplus(outdata, n))
		die_nomem();
	for (;;) {
		if (i == 4) {
			for (j = 2; j >= 0; --j) {
			...
			if (!stralloc_cats(outdata, holdch))
				die_nomem();
			if (cp >= cpnext)
				break;
			hold32 = 0L;
