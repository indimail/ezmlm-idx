/*
 * $Id: ezmlm-dispatch.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
const char      USAGE[] = "ezmlm-dispatch: usage: ezmlm-dispatch dir [list]";
...
static const char *
makepath(const char *fn)
{
	if (!stralloc_copy(&path, &basedir))
		die_nomem();
	if (fn) {
		if (!stralloc_append(&path, "/"))
			die_nomem();
		if (!stralloc_cats(&path, fn))
			die_nomem();
	}
	if (!stralloc_0(&path))
		die_nomem();
...
static int
is_dir(const char *fn)
{
	struct stat     st;
	return wrap_stat(makepath(fn), &st) == 0 && S_ISDIR(st.st_mode);
}
...
{
	struct stat     st;
	return wrap_stat(makepath(fn), &st) == 0 && S_ISREG(st.st_mode);
}

static void
forward(const char *rcpt)
{
...
static int
execute_line(const char *fn, const char *line)
{
	int             child;
	int             code;
	if (seek_begin(0) == -1)
...
static int
execute_file(const char *fn, stralloc *file)
{
	unsigned int    start;
	unsigned int    end;
	unsigned int    len;
 ...
 */
static void
execute(const char *fn, const char *def)
{
	char            strnum[FMT_ULONG];
	stralloc        file = { 0, 0, 0 };
	int             code;
 ...
 */
static void
try_dispatch(const char *def, const char *prefix, unsigned int len, const char *fn)
{
	if (str_diffn(def, prefix, len) == 0 && is_file(fn))
		execute(fn, def + len);
}
 ...
 */
static void
dispatch(const char *dir, const char *def)
{
	if (!is_dir(dir))
		return;
	if (!stralloc_append(&basedir, "/"))
		die_nomem();
	if (!stralloc_cats(&basedir, dir))
		die_nomem();
	/*
	...
	if (str_diff(def, "owner") == 0)
		execute("owner", 0);
	else
	if (str_diff(def, "digest-owner") == 0)
		execute("owner", 0);
	else {
...
main(int argc, char **argv)
{
	...
	if (argv[optind] == 0)
		die_usage();
	if (!stralloc_copys(&basedir, argv[optind++]))
		die_nomem();
