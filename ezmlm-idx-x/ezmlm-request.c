/*
 * $Id: ezmlm-request.c,v 1.1 2025-01-22 10:52:36+05:30 Cprogrammer Exp mbhangui $
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
const char      USAGE[] = "ezmlm-request: usage: ezmlm-request [-f lists.cfg] dir";
 ...
 */
static const char cmdstring[] = "system\\help\\"	/* 1,2 */
...
main(int argc, char **argv)
{
	...
	if (cfname) { /* just list-mdomo */
		local = env_get("LOCAL");
		if (!local)
			strerr_die2x(100, FATAL, MSG(ERR_NOLOCAL));
		len = str_len(local);
	...
	if (!cfname) { /* expect request */
		...
		if (case_starts(action, ALT_REQUEST))
	...
	if (!stralloc_copy(&listname, &outlocal))
		die_nomem();
	if (!stralloc_copy(&hostname, &outhost))
		die_nomem();
	if (!stralloc_0(&listname))
		die_nomem();
	...
	sender = get_sender();
	if (!sender)
		strerr_die2x(99, INFO, MSG(ERR_NOSENDER));
	if (!*sender)
	...
	if (!stralloc_copys(&mydtline, "Delivered-To: request processor for "))
		die_nomem();
	if (!stralloc_cat(&mydtline, &outlocal))
		die_nomem();
	if (!stralloc_cats(&mydtline, "@"))
		die_nomem();
	if (!stralloc_cat(&mydtline, &outhost))
		die_nomem();
	if (!stralloc_cats(&mydtline, "\n"))
		die_nomem();
	...
	if (action[0]) {			/* mainly to allow ezmlm-lists or ezmlm-which with */
		...
		for (;;) { /* Get Subject: */
			...
			if (case_startb(line.s, line.len, "content-type:")) {
				pos = 13;
				last = line.len - 2; /* not cont-line - ok */
				while (pos <= last && (line.s[pos] == ' ' || line.s[pos] == '\t'))
					++pos;
		...
		if (cfname || flagnosubject) {
			for (;;) { /* parse body */
				...
				if (flagmultipart == 2) {	/* skip content info */
					if (line.len != 1)
						continue;
	...
	if (!stralloc_copys(&cmds, cmdstring))
		die_nomem();
	if (!stralloc_0(&cmds))
		die_nomem();
	psz = cmds.s;
	...
	if (!constmap_init(&commandmap, cmds.s, cmds.len, 0))
		die_nomem();
 ...
 */
	...
	if (cmdidx >= 0) {
		...
		if (!cfname || (listhost && !case_diffs(listhost, hostname.s)))
			...
			for (;;) {
				...
				if (line.len <= 1 || line.s[0] == '#')
					continue;
				if ((pos < line.len) && (line.s[pos] == '@') && !byte_diff(line.s, pos, listlocal)) {
					last = byte_chr(line.s, line.len, ':');
					if (!stralloc_copyb(&lhost, line.s + pos + 1, last - pos - 1))
						die_nomem();
					if (!stralloc_0(&lhost))
						die_nomem();
					if (listhost) {
		...
		if (!stralloc_copys(&to, listlocal))
			die_nomem();
		if (!stralloc_cats(&to, "-"))
			die_nomem();
		if (cmdidx) { /* recognized - substitute */
			if (!stralloc_cats(&to, constmap_get(&commandmap, cmdidx)))
				die_nomem();
		} else /* not recognized - use as is */ if (!stralloc_cats(&to, command))
			die_nomem();

		if (!stralloc_cats(&to, "-"))
			die_nomem();
		if (!stralloc_cats(&to, userlocal))
			die_nomem();
		if (userhost) { /* doesn't exist for e.g. -get */
			if (!stralloc_cats(&to, "="))
				die_nomem();
			if (!stralloc_cats(&to, userhost))
				die_nomem();
		}
		if (!stralloc_cats(&to, "@"))
			die_nomem();
		if (!stralloc_cats(&to, listhost))
			die_nomem();
		if (!stralloc_0(&to))
			die_nomem();
		...
		if (userlocal) {
			if (!stralloc_copys(&to, userlocal))
				die_nomem();
			if (!stralloc_cats(&to, "@"))
				die_nomem();
			if (userhost) {
				...
				if (!stralloc_cat(&to, &outhost))
					die_nomem();
			}
		} else
		if (!stralloc_copys(&to, sender))
			die_nomem();
		if (!stralloc_0(&to))
			die_nomem();
