...
static int
readit(stralloc *sa, const char *fn)
{
	int             i;
	unsigned int    j;
	int             k;
...
const char     *
messages_getn(const char *msg, const char *params[10])
{
	const char     *xmsg;
	int             msg_len;
	params[0] = msg;
	...
	msg_len = str_len(msg);
	if (msg_local.map.num == 0 || (xmsg = constmap(&msg_local.map, msg, msg_len)) == 0)
		if (msg_alt.map.num == 0 || (xmsg = constmap(&msg_alt.map, msg, msg_len)) == 0)
			if (msg_default.map.num == 0 || (xmsg = constmap(&msg_default.map, msg, msg_len)) == 0)
				if ((xmsg = constmap(&msg_internal.map, msg, msg_len)) == 0)
					xmsg = msg;

	if (!stralloc_copys(&data, xmsg))
		die_nomem();
	if (!stralloc_catb(&data, ": ", 2))
		die_nomem();
...
const char     *
messages_get1(const char *msg, const char *p1)
{
...
const char     *
messages_get2(const char *msg, const char *p1, const char *p2)
{
