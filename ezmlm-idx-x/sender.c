...
decode_srs(const char *s)
{
	...
	if (!stralloc_copyb(&realsender, s + sep2, at - sep2))
		die_nomem();
	if (!stralloc_append(&realsender, "@"))
		die_nomem();
	if (!stralloc_catb(&realsender, s + sep1, sep2 - sep1 - 1))
		die_nomem();
	if (!stralloc_0(&realsender))
		die_nomem();
	return 1;
}
 ...
 */
static int
is_batv_tag(const char *s, int len)
{
	if (len >= 9) {
		for (; len > 0; --len, ++s) {
			if (!is_hex(*s))
...
decode_prvs(const char *s)
{
	...
	if (is_batv_tag(s + 5, sep - 5)) {
		if (!stralloc_copys(&realsender, s + sep + 1))
			die_nomem();
		if (!stralloc_0(&realsender))
			die_nomem();
		return 1;
	...
	if (is_batv_tag(s + sep + 1, at - sep - 1)) {
		if (!stralloc_copyb(&realsender, s + 5, sep - 5))
			die_nomem();
		if (!stralloc_cats(&realsender, s + at))
			die_nomem();
		if (!stralloc_0(&realsender))
			die_nomem();
		return 1;
		...
		for (sep = at - 1; sep > 6; --sep) {
			if (s[sep] == '=' && s[sep + 1] == '=') {
				if (is_batv_tag(s + 6, sep - 6)) {
					if (!stralloc_copys(&realsender, s + sep + 2))
						die_nomem();
					if (!stralloc_0(&realsender))
						die_nomem();
					return 1;
