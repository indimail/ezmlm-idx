/*
 * $Id: ezmlm-idx.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
const char      USAGE[] = "ezmlm-idx: usage: ezmlm-idx [-dDF] [-f msg] dir";
...
idx_get_trimsubject(int fd)
{
	...
	for (;;) {
		...
		if (match) {
			...
			if (!foundfrom && case_startb(line.s, line.len, "From:")) {
				if (!stralloc_copyb(&author, line.s + 5, line.len - 5))
					die_nomem();
				foundfrom = 1;
			} else
			if (!flagdate && !foundreceived && case_startb(line.s, line.len, "Received:")) {
			...
			if (flagdate && !foundreceived && case_startb(line.s, line.len, "Date:")) {
				if (line.len < 22)
					continue;	/* illegal */
				...
				if (pos1 + 2 < line.len) {
					if (line.s[pos1 + 2] == ' ') {	/* 2-digit */
						if (line.s[pos1] >= '7') {	/* >= 70 */
							if (!stralloc_cats(&received, "19"))
								die_nomem();
						} else
						if (!stralloc_cats(&received, "20"))
							die_nomem();
						pos = pos1 + 3;	/* 2 digit */
					} else
						pos = pos1 + 5;	/* 4 digit */
	...
	while (++msgnum <= msgmax) {
		if (msgnum == 1 || !(msgnum % 100)) {
			...
			if (!stralloc_catb(&fnadir, strnum, fmt_ulong(strnum, msgnum / 100)))
				die_nomem();
			if (!stralloc_copy(&fnifn, &fnadir))
				die_nomem();
			if (!stralloc_copy(&fnif, &fnadir))
				die_nomem();
			if (!stralloc_cats(&fnif, "/index"))
				die_nomem();
			if (!stralloc_cats(&fnifn, "/indexn"))
				die_nomem();
		...
		if (!stralloc_copys(&fnaf, fnadir.s))
			die_nomem();
		...
		if (fstat(fd, &st) == -1 || (!(st.st_mode & 0100)))
			...
			close(fd);
			if (!stralloc_copyb(&line, strnum, fmt_ulong(strnum, msgnum)))
				die_nomem();
			if (!stralloc_cats(&line, ": "))
				die_nomem();
			makehash(subject.s, subject.len, hash);
			if (!stralloc_catb(&line, hash, HASHLEN))
				die_nomem();
			if (!stralloc_cats(&line, " "))
				die_nomem();
			if (r & 1)			/* reply */
				if (!stralloc_cats(&line, "Re: "))
					die_nomem();
			if (!stralloc_cat(&line, &subject))
				die_nomem();
			if (!stralloc_cats(&line, "\n\t"))
				die_nomem();
			if (!stralloc_cat(&line, &received))
				die_nomem();
			if (!stralloc_cats(&line, ";"))
				die_nomem();
		...
		if (!((msgnum + 1) % 100) || (msgnum == msgmax)) {	/* last in this set */
