/*
 * $Id: ezmlm-weed.c,v 1.1 2025-01-22 10:52:36+05:30 Cprogrammer Exp mbhangui $
 */
#include <unistd.h>
#include "stralloc.h"
#include "str.h"
...
const char      USAGE[] = "ezmlm-weed: usage: ezmlm-weed";
...
main(void)
{
	...
	if (dsnline.len > 0) {		/* always only one recipient/action */
		...
		for (i = 0; i < dsnline.len; i += 1 + byte_chr(dsnline.s + i, dsnline.len - i, ';')) {
			...
			if (case_startb(dsnline.s + i, dsnline.len - i, "boundary=")) {
				i += 9;
				while (dsnline.s[i] == ' ' || dsnline.s[i] == '\t')
					if (++i >= dsnline.len)
						break;
				if (dsnline.s[i] == '"') {
					...
					j = i;
					while (dsnline.s[j] != ' ' && dsnline.s[j] != '\t' && dsnline.s[j] != ';')
						if (++j >= dsnline.len)
							break;
				} /* got boundary */
				if (!stralloc_copyb(&boundary, dsnline.s + i, j - i))
					die_nomem();
			}
		}
	}
