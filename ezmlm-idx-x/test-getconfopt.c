...
const char      USAGE[] = "test-getconfopt: usage: test-getconfopt [-aAbBvV] dir";
...
static void
put_s(const char *prefix, const char *s)
{
...
static void
put_sa(const char *prefix, const stralloc *sa)
{
	substdio_puts(&ssout, prefix);
	if (sa->s) {
		substdio_puts(&ssout, "\"");
...
static void
put_u(const char *prefix, unsigned long u)
{
	char            num[FMT_ULONG];
	substdio_puts(&ssout, prefix);
