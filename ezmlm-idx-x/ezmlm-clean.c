/*
 * $Id: ezmlm-clean.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
const char      USAGE[] = "ezmlm-clean: usage: ezmlm-clean [-mMrRvV] dir";
 ...
 */
static void
dodir(const char *dirname, int reply)
{
	DIR            *moddir;
	direntry       *d;
	...
	while ((d = readdir(moddir))) {
		...
		if (modtime < older) {
			if (!stralloc_copys(&fnmsg, dirname))
				die_nomem();
			if (!stralloc_cats(&fnmsg, d->d_name))
				die_nomem();
			if (!stralloc_0(&fnmsg))
				die_nomem();
