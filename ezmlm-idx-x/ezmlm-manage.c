/*
 * $Id: ezmlm-manage.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
const char      USAGE[] = "ezmlm-manage: usage: ezmlm-manage [-aAbBcCdDeEfFlLmMnNqQsSuUvV] dir";
...
static void
showact(const char *x1, const char *x2, const char *x3)
{
	if (flagshowact)
...
static void
showsend(const char *t)
{
	showact("sending ", t, 0);
}

static void
showsend2(const char *t1, const char *t2)
{
	showact("sending ", t1, t2);
}

static void
copy_act(const char *text)
{
	showsend(text + 5);
	copy(&qq, text, flagcd);
}
 ...
 */
void
make_verptarget(void)
{
	unsigned int    i;

	i = str_rchr(target.s, '@');
	if (!stralloc_copyb(&verptarget, target.s, i))
		die_nomem();
	if (target.s[i]) {
		if (!stralloc_append(&verptarget, "="))
			die_nomem();
		if (!stralloc_cats(&verptarget, target.s + i + 1))
			die_nomem();
	}
	if (!stralloc_0(&verptarget))
		die_nomem();
 ...
 */
void
store_from(stralloc *frl, /* from line */ const char *adr)
{
	int             fdin;
	int             fdout;
	unsigned long   linetime;

	if (!flagstorefrom || !frl->len)
	...
	if ((fdin = open_read("from")) == -1) {
		...
		for (;;) {
			...
			if (linetime + 1000000 > (unsigned long) when && linetime <= (unsigned long) when)
				if (substdio_bput(&ssfrom, line.s, line.len))
					strerr_die2sys(111, FATAL, MSG1(ERR_WRITE, "fromn"));
		}
		close(fdin);
	} /* build new entry */
	if (!stralloc_copyb(&line, strnum, fmt_ulong(strnum, when)))
		die_nomem();
	if (!stralloc_append(&line, " "))
		die_nomem();
	if (!stralloc_cats(&line, adr))
		die_nomem();
	if (!stralloc_0(&line))
		die_nomem();
	if (!stralloc_catb(&line, frl->s, frl->len))
		die_nomem();
	if (!stralloc_append(&line, "\n"))
		die_nomem();
 ...
 */
const char     *
get_from(const char *adr, /* target address */ const char *act /* action */)
{							
	int             fd;
	const char     *fl;
	unsigned int    pos;
...
int
hashok(const char *action, const char *ac)
{
	const char     *x;
	datetime_sec    u;

	x = action + 3;
	x += scan_ulong(x, (unsigned long *) &u);
	hashdate = u;
	if (hashdate > when)
		return 0;
	if (hashdate + 1000000 < when)
		return 0;
...
int
code_qput(const char *s, unsigned int n)
{
	if (!flagcd)
		qmail_put(&qq, s, n);
	else {
...
int
code_qputs(const char *s)
{
	return code_qput(s, str_len(s)) && code_qput("\n", 1);
}

int
subto(const char *s, unsigned int l)
{
	qmail_put(&qq, "T", 1);
	qmail_put(&qq, s, l);
	qmail_put(&qq, "", 1);
...
int
code_subto(const char *s, unsigned int l)
{
	code_qput(s, l);
	code_qput("\n", 1);
	return (int) l;
...
int
dummy_to(const char *s, /* ignored */ unsigned int l)
{
	return (int) l;
	(void) s;
}
...
to_owner(void)
{
	if (!stralloc_copy(&owner, &outlocal))
		die_nomem();
	if (!stralloc_cats(&owner, "-owner@"))
		die_nomem();
	if (!stralloc_cat(&owner, &outhost))
		die_nomem();
	if (!stralloc_0(&owner))
		die_nomem();
	qmail_to(&qq, owner.s);
...
msg_headers(int act)
{
	 ...
	 */
	flaggetfrom = flagstorefrom && ((act == AC_SC) || ((act == AC_SUBSCRIBE) && !flagsubconf));
	for (;;) {
		...
		if (case_startb(line.s, line.len, "content-transfer-encoding:")) {
			...
			if (case_startb(line.s + pos, line.len - pos, "quoted-printable"))
				encin = 'Q';
		} else
		if (flaggetfrom && case_startb(line.s, line.len, "from:")) {
			/*
...
geton(const char *action)
{
	...
	case 1:
		...
		if (!stralloc_copy(&confirm, &outlocal))
			die_nomem();
		if (!stralloc_cats(&confirm, "-unsubscribe-"))
			die_nomem();
		if (!stralloc_cats(&confirm, verptarget.s))
			die_nomem();
		if (!stralloc_append(&confirm, "@"))
			die_nomem();
...
doconfirm(const char *act)
{
	...
	if (!stralloc_append(&confirm, "-"))
		die_nomem();
	if (!stralloc_catb(&confirm, act, 1))
		die_nomem();
...
static void
do_subscribe(const char *action)
{
	int             r;
	...
	if (modsub.s != 0) {
		...
		if (flagnotify)
			qmail_to(&qq, target.s);
		if (r && flagcopyowner > 1)
			to_owner();
	}
}

static void
do_sc(const char *action)
{
	int             r;
...
static void
do_rc_tc(const char *action, const char *ac)
{
	int             r;
	...
	if (hashok(action, ac)) {
		...
		mod_bottom();
		if (flagnotify)
			qmail_to(&qq, target.s);	/* unless suppressed */
		if (r && flagcopyowner > 1)
			to_owner();
	} else {
...
static void
do_unsubscribe(const char *action)
{
	int             r;
	...
	if (flagunsubismod && modsub.s != 0) {
		 ...
		 */
		if (flagcopyowner && (!r || flagcopyowner > 1))
			to_owner();
	}
}

static void
do_uc(const char *action)
{
	int             r;
...
static void
do_vc_wc(const char *action, const char *ac)
{
	int             r;
...
static void
do_log(char *action, unsigned int actlen)
{
	action += actlen;
	if (*action == '.' || *action == '_')
		++action;
...
do_edit(const char *action)
{
	...
	if (action[len]) { /* -edit.file, not just -edit */
		...
		if (!stralloc_copys(&fnedit, "text/"))
			die_nomem();
		if (!stralloc_cats(&fnedit, action + len + 1))
			die_nomem();
		if (!stralloc_0(&fnedit))
			die_nomem();
		...
		if (!stralloc_copy(&line, &text))
			die_nomem();

		subst_nuls(&line);

		if (!stralloc_cat(&line, &fnedit))
		...
		if (!stralloc_append(&confirm, "-"))
			die_nomem();
		if (!stralloc_catb(&confirm, ACTION_ED, LENGTH_ED))
			die_nomem();
		if (!stralloc_cats(&confirm, strnum))
			die_nomem();
		if (!stralloc_append(&confirm, "."))
		...
		if (!stralloc_cats(&confirm, action + len + 1))
			die_nomem();
		if (!stralloc_append(&confirm, "."))
			die_nomem();
		if (!stralloc_catb(&confirm, hash, COOKIE))
...
do_ed(char *action)
{
	...
	*x = (char) 0;
	...
	if (str_len(x) != COOKIE)
		die_cookie();
	if (byte_diff(hash, COOKIE, x))
		die_cookie();
	/*
	 * cookie is ok, file exists, lock's on, new file ends in '_' 
	 */
	if (!stralloc_copys(&fneditn, fnedit.s))
		die_nomem();
	if (!stralloc_append(&fneditn, "_"))
		die_nomem();
	if (!stralloc_0(&fneditn))
		die_nomem();
	...
	while ((cpnext = cp + byte_chr(cp, cpafter - cp, '\n')) != cpafter) {
		...
		if (flaggoodfield) {
			...
			if (quoted.len && cpnext - cp >= (int) quoted.len && !str_diffn(cp, quoted.s, quoted.len))
...
static char    *
set_workdir(char *action)
{
	if (case_starts(action, "digest")) { /* digest */
		action += 6;
		if (!stralloc_cats(&outlocal, "-digest"))
			die_nomem();
		workdir = "digest";
		flagdig = FLD_DIGEST;
	} else
	if (case_starts(action, ACTION_ALLOW)) { /* allow */
		action += str_len(ACTION_ALLOW);
		if (!stralloc_append(&outlocal, "-"))
			die_nomem();
		if (!stralloc_cats(&outlocal, ACTION_ALLOW))
			die_nomem();
		workdir = "allow";
		flagdig = FLD_ALLOW;
	} else
	if (case_starts(action, ACTION_DENY)) { /* deny */
		action += str_len(ACTION_DENY);
		if (!stralloc_append(&outlocal, "-"))
			die_nomem();
		if (!stralloc_cats(&outlocal, ACTION_DENY))
			die_nomem();
		workdir = "deny";
		flagdig = FLD_DENY;
	} else {
...
static int
get_act_ismod(const char *action, unsigned int *actlen)
{
	int             act;

	if (case_equals(action, ACTION_LISTN) || case_equals(action, ALT_LISTN))
		act = AC_LISTN;
	else
	if (case_equals(action, ACTION_LIST) || case_equals(action, ALT_LIST))
		act = AC_LIST;
	else
	if (case_starts(action, ACTION_GET) || case_starts(action, ALT_GET))
		act = AC_GET;
	else
	if (case_equals(action, ACTION_HELP) || case_equals(action, ALT_HELP))
		act = AC_HELP;
	else
	if (case_starts(action, ACTION_EDIT) || case_starts(action, ALT_EDIT))
		act = AC_EDIT;
	else
	if (case_starts(action, ACTION_LOG)) {
		act = AC_LOG;
		*actlen = str_len(ACTION_LOG);
	} else
	if (case_starts(action, ALT_LOG)) {
		act = AC_LOG;
		*actlen = str_len(ALT_LOG);
	} else
		act = AC_NONE;

	if (modsub.s != 0 || remote.s != 0) {
		if (modsub.len) {
			if (!stralloc_copy(&moddir, &modsub))
				die_nomem();
		} else
		if (remote.len) {
			if (!stralloc_copy(&moddir, &remote))
				die_nomem();
		} else {
			if (!stralloc_copys(&moddir, "mod"))
				die_nomem();
		}
		if (!stralloc_0(&moddir))
			die_nomem();
		/*
	...
	if (!flagpublic && !(ismod && remote.s != 0) && !case_equals(action, ACTION_HELP))
	...
	if (act == AC_NONE) {		/* none of the above */
		if (case_equals(action, ACTION_SUBSCRIBE) || case_equals(action, ALT_SUBSCRIBE))
			act = AC_SUBSCRIBE;
		else
		if (case_equals(action, ACTION_UNSUBSCRIBE)
				 || case_equals(action, ALT_UNSUBSCRIBE))
			act = AC_UNSUBSCRIBE;
		else
		if (str_start(action, ACTION_SC))
			act = AC_SC;
	}
	return act;
}
...
main(int argc, char **argv)
{
	...
	if (str_start(action, ACTION_RC))
		do_rc_tc(action, ACTION_RC);
	else
	if (str_start(action, ACTION_TC))
		do_rc_tc(action, ACTION_TC);
	else
	if (act == AC_UNSUBSCRIBE)
		do_unsubscribe(action);
	else
	if (str_start(action, ACTION_UC))
		do_uc(action);
	else
	if (str_start(action, ACTION_VC))
		do_vc_wc(action, ACTION_VC);
	else
	if (str_start(action, ACTION_WC))
		do_vc_wc(action, ACTION_WC);
	else
	if (act == AC_LIST || act == AC_LISTN)
		do_list(act);
	else
	if (act == AC_LOG)
	...
	if (str_start(action, ACTION_ED))
		do_ed(action);
	else
	if (act == AC_GET)
		do_get(action);
	else
	if (case_starts(action, ACTION_QUERY) || case_starts(action, ALT_QUERY))
		do_query();
	else
	if (case_starts(action, ACTION_INFO) || case_starts(action, ALT_INFO))
		do_info();
	else
	if (case_starts(action, ACTION_FAQ) || case_starts(action, ALT_FAQ))
		do_faq();
	else
	if (ismod && (act == AC_HELP))
		do_mod_help();
	else
		do_help();
