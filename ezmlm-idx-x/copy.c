/*
 * $Id: copy.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 *
...
void
set_cpverptarget(const char *tg)
{
	verptarget = tg;
}

void
set_cpconfirm(const char *cf, unsigned int prefixlen)
{
	confirm = cf;
...
void
set_cpnum(const char *cf)
{
	szmsgnum = cf;
}

void
set_cpaction(const char *ac)
{
	action = ac;
}

void
set_cpwhen(datetime_sec t)
{
	when = t;
}

void
set_cphash(const char h[COOKIE])
{
	byte_copy(hash, COOKIE, h);
}

void
codeput(struct qmail *qq, const char *l, unsigned int n, char code)
{
	if (!code || code == 'H')
		qmail_put(qq, l, n);
	else {
...
void
codeputs(struct qmail *qq, const char *l, char code)
{
 ...
 */
void
copy_xlate(stralloc *out, const stralloc *line, const char *params[10], char q)
{
	unsigned int    pos;
	...
	while ((pos += byte_chr(line->s + pos, line->len - pos, '<')) < line->len) {
		if (pos + 4 < line->len && line->s[pos + 1] == '#' && line->s[pos + 3] == '#' && line->s[pos + 4] == '>') {
			...
			case 'A':
				if (q == 'H')
					strerr_die1x(111, MSG(ERR_SUBST_UNSAFE));
				if (!stralloc_cats(out, target))
					die_nomem();
				break;
			case 'C':
				if (!stralloc_catb(out, hash, COOKIE))
					die_nomem();
				break;
			case 'L':
				if (!quote(&qline, &mainlocal))
					die_nomem();
				if (!stralloc_cat(out, &qline))
					die_nomem();
				break;
			case 'R':
				if (!stralloc_cats(out, confirm))
					die_nomem();
				break;
			case 'T':
				if (!stralloc_catb(out, strnum, fmt_ulong(strnum, (unsigned long) when)))
					die_nomem();
				break;
			case 'X':
				if (!stralloc_cats(out, action))
					die_nomem();
				break;
			case 'a':
				if (!stralloc_catb(out, target, targetlocal))
					die_nomem();
				break;
			case 'c':
				if (!stralloc_catb(out, confirm + confirmprefix, confirmlocal - confirmprefix))
					die_nomem();
				break;
			case 'd':
				if (listdir != 0)
					if (!stralloc_cats(out, listdir))
						die_nomem();
				break;
			case 'r':
				if (!stralloc_catb(out, confirm, confirmlocal))
					die_nomem();
				break;
			case 'l':
				if (!quote(&qline, &outlocal))
					die_nomem();
				if (!stralloc_cat(out, &qline))
					die_nomem();
				break;
			case 'h':
			case 'H':
				if (!stralloc_cat(out, &outhost))
					die_nomem();
				break;
			case 't':
				if (q == 'H')
					strerr_die1x(111, MSG(ERR_SUBST_UNSAFE));
				if (!stralloc_cats(out, verptarget))
					die_nomem();
				break;
			case 'n':
				if (!stralloc_cats(out, szmsgnum))
					die_nomem();
				break;
			case '0':
			case '1':
...
/*
 * '\0' for regular output, 'B' for base64,
 * 'Q' for quoted printable,'H' for header
 */
void
copy(struct qmail *qq, const char *fn,	/* text file name */ char q)
{
	int             fd;
	int             flagsmatched = 1;
	int             match;
	...
	if ((fd = alt_open_read(fn)) == -1)
		strerr_die2sys((errno == error_noent) ? 100 : 111, FATAL, MSG1(ERR_OPEN, fn));
	...
	for (;;) {
		...
		if (srcline.len > 0) {	/* srcline.len is always > 0 if match is true */
			...
			/*- Find <=flags=> tags */
			if (q != 'H' && srcline.len >= 5 && srcline.s[0] == '<' && srcline.s[1] == '=' && srcline.s[srcline.len - 3] == '='
				&& srcline.s[srcline.len - 2] == '>') {
				for (flagsmatched = 1, pos = 2; flagsmatched && pos < srcline.len - 3; ++pos)
