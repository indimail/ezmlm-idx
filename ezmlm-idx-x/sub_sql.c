...
static void
make_name(struct subdbinfo *info, const char *suffix1, const char *suffix2, int terminate)
{
	if (!stralloc_copys(&name, info->base_table))
		die_nomem();
 ...
 */
const char     *
sub_sql_checktag(struct subdbinfo *info, unsigned long num,	/* message number */
				 unsigned long listno,	/* bottom of range => slave */
				 const char *action, const char *seed, const char *hash /* cookie */)
{
	void           *result;
	...
	if (listno) { /* only for slaves */
		if (!stralloc_copyb(&params[0], strnum, fmt_ulong(strnum, listno)))
			die_nomem();
		if (!stralloc_copyb(&params[1], strnum, fmt_ulong(strnum, num)))
			die_nomem();
		if (!stralloc_copys(&query, "SELECT listno FROM "))
			die_nomem();
		if (!stralloc_cats(&query, info->base_table))
			die_nomem();
		if (!stralloc_cats(&query, "_mlog WHERE "))
			die_nomem();
		if (!stralloc_cats(&query, sql_checktag_listno_where_defn))
			die_nomem();
	...
	if (!stralloc_copyb(&params[0], strnum, fmt_ulong(strnum, num)))
		die_nomem();
	if (!stralloc_copyb(&params[1], hash, COOKIE))
		die_nomem();

	if (!stralloc_copys(&query, "SELECT msgnum FROM "))
		die_nomem();
	if (!stralloc_cats(&query, info->base_table))
		die_nomem();
	if (!stralloc_cats(&query, "_cookie WHERE "))
		die_nomem();
	if (!stralloc_cats(&query, sql_checktag_msgnum_where_defn))
		die_nomem();
...
int
sub_sql_issub(struct subdbinfo *info, const char *table, const char *userhost, stralloc *recorded)
{
	unsigned int    j;
	void           *result;
	int             ret;
 ...
 */
	...
	if (!stralloc_copys(&addr, userhost))
		die_nomem();
	...
	if (!stralloc_copys(&query, "SELECT address FROM "))
		die_nomem();
	if (!stralloc_cat(&query, &name))
		die_nomem();
	if (!stralloc_cats(&query, " WHERE "))
		die_nomem();
	if (!stralloc_cats(&query, sql_issub_where_defn))
		die_nomem();
	...
	if (!sql_fetch_row(info, result, 1, &addr))
		 ...
		 */
		if (recorded != 0) {
			if (!stralloc_copy(recorded, &addr))
				die_nomem();
			if (!stralloc_0(recorded))
				die_nomem();
		}
		ret = 1;
	}
 ...
 */
const char     *
sub_sql_logmsg(struct subdbinfo *info, unsigned long num, unsigned long listno, unsigned long subs, int done)
{
	char           *s;
	char            strnum[FMT_ULONG];

	if (!stralloc_copys(&query, "INSERT INTO "))
		die_nomem();
	if (!stralloc_cats(&query, info->base_table))
		die_nomem();
	if (!stralloc_cats(&query, "_mlog (msgnum,listno,subs,done) VALUES "))
		die_nomem();
	if (!stralloc_cats(&query, sql_logmsg_values_defn))
		die_nomem();
	if (!stralloc_copyb(&params[0], strnum, fmt_ulong(strnum, num)))
		die_nomem();
	if (!stralloc_copyb(&params[1], strnum, fmt_ulong(strnum, listno)))
		die_nomem();
	if (!stralloc_copyb(&params[2], strnum, fmt_ulong(strnum, subs)))
		die_nomem();
	s = strnum;
	if (done < 0) {
		done = -done;
 ...
 */
unsigned long
sub_sql_putsubs(struct subdbinfo *info, const char *table, unsigned long hash_lo, unsigned long hash_hi,
				int subwrite(const char *, unsigned int) /* write function. */)
{
	...
	if (!stralloc_copyb(&params[0], strnum, fmt_ulong(strnum, hash_lo)))
		die_nomem();
	if (!stralloc_copyb(&params[1], strnum, fmt_ulong(strnum, hash_hi)))
		die_nomem();
	make_name(info, table ? "_" : 0, table, 0);
	...
	if (!stralloc_cats(&query, " WHERE "))
		die_nomem();
	if (!stralloc_cats(&query, sql_putsubs_where_defn))
		die_nomem();
 ...
 */
void
sub_sql_searchlog(struct subdbinfo *info, const char *table, char *search,	/* search string */
				  int subwrite    (const char *, unsigned int) /* output fxn */)
{
	...
	if (!stralloc_copys(&query, "SELECT "))
		die_nomem();
	if (!stralloc_cats(&query, sql_searchlog_select_defn))
		die_nomem();
	if (!stralloc_cats(&query, " FROM "))
		die_nomem();
	if (!stralloc_cat(&query, &name))
		die_nomem();
	if (!stralloc_cats(&query, "_slog"))
		die_nomem();
	if (*search) { /* We can afford to wait for LIKE '%xx%' */
		if (!stralloc_copys(&params[0], search))
			die_nomem();
		if (!stralloc_copys(&params[1], search))
			die_nomem();
		nparams = 2;
		if (!stralloc_cats(&query, " WHERE "))
			die_nomem();
		if (!stralloc_cats(&query, sql_searchlog_where_defn))
			die_nomem();
	} else
		nparams = 0;
	/*- ordering by tai which is an index */
	if (!stralloc_cats(&query, " ORDER by tai"))
		die_nomem();

	result = sql_select(info, &query, nparams, params);
	while (sql_fetch_row(info, result, 2, params)) {
		if (!stralloc_0(&params[0]))
			die_nomem();
		(void) scan_ulong(params[0].s, (unsigned long *) &when);
		datetime_tai(&dt, when);
		if (!stralloc_copyb(&params[0], date, date822fmt(date, &dt) - 1))
			die_nomem();
		if (!stralloc_cats(&params[0], ": "))
			die_nomem();
		if (!stralloc_catb(&params[0], strnum, fmt_ulong(strnum, when)))
			die_nomem();
		if (!stralloc_cats(&params[0], " "))
			die_nomem();
		if (!stralloc_cat(&params[0], &params[1]))
			die_nomem();
		if (subwrite(params[0].s, params[0].len) == -1)
			die_write();
	}
	sql_free_result(info, result);
}
 ...
 */
int
sub_sql_subscribe(struct subdbinfo *info, const char *table, const char *userhost, int flagadd, const char *comment,
				  const char *event, int forcehash)
{
	void           *result;
	char           *cpat;
	char            szhash[3] = "00";
	...
	if (flagadd) {
		...
		if (!stralloc_cats(&query, " WHERE "))
			die_nomem();
		if (!stralloc_cats(&query, sql_subscribe_select_where_defn))
			die_nomem();
		if (!stralloc_copy(&params[0], &addr))
			die_nomem();
		...
		if (sql_fetch_row(info, result, 1, params)) {
			...
			if (!stralloc_cat(&query, &name))
				die_nomem();
			if (!stralloc_cats(&query, " (address,hash) VALUES "))
				die_nomem();
			if (!stralloc_cats(&query, sql_subscribe_list_values_defn))
				die_nomem();
			if (!stralloc_copy(&params[0], &addr))
				die_nomem();
			if (!stralloc_copys(&params[1], szhash))
				die_nomem();
		...
		if (!stralloc_copys(&query, "DELETE FROM "))
			die_nomem();
		if (!stralloc_cat(&query, &name))
			die_nomem();
		if (!stralloc_cats(&query, " WHERE "))
		...
		if (!stralloc_copy(&params[0], &addr))
			die_nomem();
		if (forcehash >= 0) {
			if (!stralloc_cats(&query, sql_subscribe_delete2_where_defn))
				die_nomem();
			if (!stralloc_copys(&params[1], szhash))
				die_nomem();
		} else {
			if (!stralloc_cats(&query, sql_subscribe_delete1_where_defn))
				die_nomem();
		}
	...
	if (!stralloc_cat(&query, &name))
		die_nomem();
	if (!stralloc_cats(&query, "_slog (address,edir,etype,fromline) VALUES "))
		die_nomem();
	if (!stralloc_cats(&query, sql_subscribe_slog_values_defn))
		die_nomem();
	if (!stralloc_copy(&params[0], &addr))
		die_nomem();
	if (!stralloc_copys(&params[1], flagadd ? "+" : "-"))
		die_nomem(); /* edir */
	if (!stralloc_copyb(&params[2], event + 1, !!*(event + 1)))
		die_nomem(); /* etype */
	if (!stralloc_copys(&params[3], comment && *comment ? comment : ""))
		die_nomem(); /* from */
 ...
 */
void
sub_sql_tagmsg(struct subdbinfo *info, unsigned long msgnum,	/* number of this message */
			   const char *hashout,	/* previously calculated hash */
			   unsigned long bodysize, unsigned long chunk)
{
	const char     *ret;
	char            strnum[FMT_ULONG];
	...
	if (!stralloc_cats(&query, "_cookie (msgnum,tai,cookie,bodysize,chunk) VALUES "))
		die_nomem();
	if (!stralloc_cats(&query, sql_tagmsg_values_defn))
		die_nomem();
	if (!stralloc_copyb(&params[0], strnum, fmt_ulong(strnum, msgnum)))
		die_nomem();
	if (!stralloc_copyb(&params[1], hashout, COOKIE))
		die_nomem();
	if (!stralloc_copyb(&params[2], strnum, fmt_ulong(strnum, bodysize)))
		die_nomem();
	if (!stralloc_copyb(&params[3], strnum, fmt_ulong(strnum, chunk)))
		die_nomem();
...
static const char *
create_table(struct subdbinfo *info, const char *suffix1, const char *suffix2, const char *definition)
{
	make_name(info, suffix1, suffix2, 1);
	if (sql_table_exists(info, name.s) > 0)
		return 0;
	if (!stralloc_copys(&query, "CREATE TABLE "))
		die_nomem();
	if (!stralloc_cats(&query, name.s))
		die_nomem();
	if (!stralloc_cats(&query, " ("))
		die_nomem();
	if (!stralloc_cats(&query, definition))
		die_nomem();
	if (!stralloc_cats(&query, ")"))
		die_nomem();
	if (!stralloc_0(&query))
...
static const char *
create_table_set(struct subdbinfo *info, const char *suffix, int do_mlog)
{
	const char     *r;
...
const char     *
sub_sql_mktab(struct subdbinfo *info)
{
	const char     *r;
...
static const char *
remove_table(struct subdbinfo *info, const char *suffix1, const char *suffix2)
{
	make_name(info, suffix1, suffix2, 1);
	if (sql_table_exists(info, name.s) == 0)
		return 0;
...
static const char *
remove_table_set(struct subdbinfo *info, const char *suffix)
{
	const char     *r;
	if ((r = remove_table(info, suffix, "_mlog")) != 0 || (r = remove_table(info, suffix, "_cookie")) != 0
		|| (r = remove_table(info, suffix, "_slog")) != 0 || (r = remove_table(info, suffix, "")) != 0)
		return r;
	return 0;
}

const char     *
sub_sql_rmtab(struct subdbinfo *info)
{
	const char     *r;
	if ((r = remove_table_set(info, "")) != 0 || (r = remove_table_set(info, "_allow")) != 0
		|| (r = remove_table_set(info, "_deny")) != 0 || (r = remove_table_set(info, "_digest")) != 0
