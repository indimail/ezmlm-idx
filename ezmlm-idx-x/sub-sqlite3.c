...
static sqlite3_stmt *
_sqlquery(const struct subdbinfo *info, const stralloc *str)
{
	sqlite3_stmt   *stmt;
...
{
	if (!(sqlite3 *) info->conn) {
		if (!stralloc_copys(&line, info->db))
			die_nomem();
		if (!stralloc_cats(&line, ".db"))
			die_nomem();
		if (!stralloc_0(&line))
			die_nomem();
...
sql_table_exists(struct subdbinfo *info, const char *name)
{
	...
	if (!stralloc_copys(&line, "SELECT name FROM sqlite_master WHERE name='"))
		return -1;
...
sql_create_table(struct subdbinfo *info, const char *definition)
{
	...
	if (!stralloc_copys(&line, definition))
		die_nomem();
	if (!stralloc_0(&line))
		die_nomem();
...
const char      sql_slog_table_defn[] =
	"  tai	INTEGER," "  address	VARCHAR(255) NOT NULL," "  fromline	VARCHAR(255) NOT NULL," "  edir	CHAR NOT NULL,"
	"  etype	CHAR NOT NULL";
...
const char      sql_cookie_table_defn[] =
	"  msgnum	INT4 NOT NULL PRIMARY KEY," "  tai	INTEGER NOT NULL," "  cookie	CHAR(20) NOT NULL,"
...
const char      sql_mlog_table_defn[] =
	"msgnum	INT4 NOT NULL," "listno	INT4 NOT NULL," "tai		TIMESTAMP," "subs		INT4 NOT NULL DEFAULT 0,"
