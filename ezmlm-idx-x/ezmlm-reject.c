/*
 * $Id: ezmlm-reject.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include <unistd.h>
#include "strerr.h"
#include "substdio.h"
...
const char      USAGE[] = "ezmlm-reject: usage: ezmlm-reject [-bBcCfFhHqQsStT] [dir]";
 ...
 */
static unsigned int
findlocal(const stralloc *sa, unsigned int n)
{
	const char     *first;
	const char     *s;
	int             level = 0;
 ...
 */
static unsigned int
findhost(const stralloc *sa, unsigned int n)
{
	const char     *last;
	const char     *s;
	int             level = 0;
...
main(int argc, char **argv)
{
	...
	for (;;) {
		...
		cp = line.s;
		len = line.len;
		if (!flagtook && (case_startb(cp, len, "to:") || case_startb(cp, len, "cc:"))) {
			if (!stralloc_catb(&to, line.s + 3, line.len - 4))
				die_nomem();
		} else
		if ((flagneedsubject || flagrejectcommands) && case_startb(cp, len, "subject:")) {
			concatHDR(cp + 8, len - 8, &subject);
		} else
		if (case_startb(cp, len, "content-type:")) {
			concatHDR(cp + 13, len - 13, &content);
		} else
		if (case_startb(cp, len, "precedence:")) {
	...
	if (precd.len >= 4 && (!case_diffb(precd.s + precd.len - 4, 4, "junk") || !case_diffb(precd.s + precd.len - 4, 4, "bulk")))
	...
	case 6:					/* Why can't they just leave an empty subject empty? */
		...
		break;
	case 9:
		if (!case_diffb("subscribe", 9, cp))
			flaghavecommand = 1;
		break;
	case 11:
		if (!case_diffb("unsubscribe", 11, cp))
			flaghavecommand = 1;
		break;
	case 12:
		if (!case_diffb("(no subject)", 12, cp))
			flaghavesubject = 0;
		break;
	default:
		break;
	...
	if (flagrejectcommands && flaghavecommand) {
		if (flagforward) {		/* flagforward => forward */
			...
			if (qmail_copy(&qq, &ssin2, copylines) != 0)
				strerr_die2sys(111, FATAL, MSG(ERR_READ_INPUT));
			if (!stralloc_copy(&to, &outlocal))
				die_nomem();
			if (!stralloc_cats(&to, "-request@"))
				die_nomem();
	...
	for (;;) {
		...
		if (flagcheck && case_startb(line.s, line.len, "content-type:")) {
			cp = line.s + 13;
			len = line.len - 14;	/* zap '\n' */
			while (*cp == ' ' || *cp == '\t') {
				++cp;
		...
		if (boundary.len && *line.s == '-' && line.len > boundary.len && !str_diffn(line.s, boundary.s, boundary.len)) {
			flagcheck = 1;
		} else {
			if (!msgsize && flagbody)
				if (case_startb(line.s, line.len, "subscribe") || case_startb(line.s, line.len, "unsubscribe"))
