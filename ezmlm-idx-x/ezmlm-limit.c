/*
 * $Id: ezmlm-limit.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
const char      USAGE[] = "ezmlm-limit: usage: ezmlm-limit [-f file] [-dDF] [-n messages] [-t secs] dir";
...
main(int argc, char **argv)
{
	...
	if (!flagloop || loopwhen + deltasecs < when || loopwhen > when) {
		...
		if (fd == -1)
			die_new();
		substdio_fdbuf(&ssnew, (ssize_t(*)(int, char *, size_t)) write, fd, newbuf, sizeof (newbuf));
		if (substdio_put(&ssnew, strnum, fmt_ulong(strnum, num)) == -1)
			die_new();
		if (substdio_puts(&ssnew, ":") == -1)
			die_new();
		if (substdio_put(&ssnew, strnum, fmt_ulong(strnum, when)) == -1)
			die_new();
	...
	if (num >= loopnum + deltanum) {	/* excess messages */
		if (!flagd) {
