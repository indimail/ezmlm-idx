...
_opensub(struct subdbinfo *info)
{
	if (!(MYSQL *) info->conn) {
		...
		if (mysql_options((MYSQL *) info->conn, MYSQL_READ_DEFAULT_GROUP, "indimail"))
			return mysql_error((MYSQL *) info->conn);
		if (!(mysql_real_connect((MYSQL *) info->conn, info->host, info->user, info->pw, info->db, info->port, 0, 0)))
			return mysql_error((MYSQL *) info->conn);
	}
	return (char *) 0;
}
...
MYSQL_STMT     *
_prepbind(struct subdbinfo *info, struct stralloc *q, unsigned int nparams, struct stralloc *params)
{
	MYSQL_STMT     *stmt;
...
sql_fetch_row(struct subdbinfo *info, void *result, unsigned int ncolumns, struct stralloc *columns)
{
	...
	for (i = 0; i < ncolumns; ++i) {
		if (!stralloc_ready(&columns[i], lengths[i]))
			die_nomem();
...
const char     *
sql_create_table(struct subdbinfo *info, const char *defn)
{
...
const char      sql_sub_table_defn[] =
	"  hash    TINYINT UNSIGNED NOT NULL," "  address VARCHAR(255) NOT NULL," "  INDEX h (hash)," "  INDEX a (address(12))";
...
const char      sql_slog_table_defn[] =
	"  tai		TIMESTAMP," "  address	VARCHAR(255) NOT NULL," "  fromline	VARCHAR(255) NOT NULL," "  edir		CHAR(1) NOT NULL,"
	"  etype	CHAR(1) NOT NULL," "  INDEX (tai)";

/*
 * main list inserts a cookie here. Sublists check it 
 */
const char      sql_cookie_table_defn[] =
	"  msgnum	INTEGER UNSIGNED NOT NULL," "  tai	TIMESTAMP NOT NULL," "  cookie	CHAR(20) NOT NULL,"
	"  chunk	TINYINT UNSIGNED NOT NULL DEFAULT 0," "  bodysize	INTEGER UNSIGNED NOT NULL DEFAULT 0," "  PRIMARY KEY (msgnum)";
...
const char      sql_mlog_table_defn[] =
	"msgnum	INTEGER UNSIGNED NOT NULL," "listno	INTEGER UNSIGNED NOT NULL," "tai		TIMESTAMP,"
	"subs		INTEGER UNSIGNED NOT NULL DEFAULT 0," "done		TINYINT NOT NULL DEFAULT 0,"
