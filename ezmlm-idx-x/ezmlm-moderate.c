/*
 * $Id: ezmlm-moderate.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
const char      USAGE[] = "ezmlm-moderate: usage: ezmlm-moderate [-cCmMrRvV] [-t replyto] dir [/path/ezmlm-send]";
 ...
 * -2 found in rejected                   
 * Handles errors.                                
 * ALSO: if found, fnmsg contains the \0-terminated
 * file name.                                     
 */
static int
checkfile(const char *fn)
{
	...
	if (!stralloc_copys(&fnmsg, "mod/pending/"))
		die_nomem();
	if (!stralloc_cats(&fnmsg, fn))
		die_nomem();
	if (!stralloc_0(&fnmsg))
		die_nomem();
	...
	if (!stralloc_copys(&fnmsg, "mod/accepted/"))
		die_nomem();
	if (!stralloc_cats(&fnmsg, fn))
		die_nomem();
	if (!stralloc_0(&fnmsg))
	...
	if (!stralloc_copys(&fnmsg, "mod/rejected/"))
		die_nomem();
	if (!stralloc_cats(&fnmsg, fn))
		die_nomem();
	if (!stralloc_0(&fnmsg))
...
maketo(void)
{
	...
	if (case_startb(line.s, line.len, "return-path:")) {
		...
		if (x != line.len) {
			...
			if (y + x != line.len) {
				if (!stralloc_copyb(&to, line.s + x + 1, y - 1))
					die_nomem();
				if (!stralloc_0(&to))
					die_nomem();
...
main(int argc, char **argv)
{
	...
	if (!action[confnum])
		die_badformat();
	if (!stralloc_copyb(&fnbase, action + start + 1, confnum - start - 1))
		die_nomem();
	if (!stralloc_0(&fnbase))
		die_nomem();
	...
	if (str_start(action, ACTION_REJECT)) {
		...
		while ((cpnext = cp + byte_chr(cp, cpafter - cp, '\n')) != cpafter) {
			...
			if (flaggoodfield) {
				...
				flagdone = 1;
				i = cplast - cp + 1;
				if (quoted.len && quoted.len <= i && !str_diffn(cp, quoted.s, quoted.len)) {	/* quote chars */
					if (!stralloc_catb(&line, cp + quoted.len, i - quoted.len))
						die_nomem();
				} else
				if (!stralloc_catb(&line, cp, i))
		...
		if (!stralloc_copys(&fnnew, "mod/rejected/"))
			die_nomem();
		if (!stralloc_cats(&fnnew, fnbase.s))
			die_nomem();
		if (!stralloc_0(&fnnew))
			die_nomem();
	...
	if (str_start(action, ACTION_ACCEPT)) {
		...
		close(fd);
		wrap_exitcode(child);
		if (!stralloc_copys(&fnnew, "mod/accepted/"))
			die_nomem();
