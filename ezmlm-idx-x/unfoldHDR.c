/*
 * $Id: unfoldHDR.c,v 1.1 2025-01-22 11:01:06+05:30 Cprogrammer Exp mbhangui $
 */
#include "stralloc.h"
#include "strerr.h"
#include "case.h"
...
static int
trimre(char **cpp, char *cpend, stralloc *prefix)
{
	int             r = 0;
	char           *cp;
	char           *cpnew;
	...
	while (junk) {
		...
		while (++cpnew <= cpend) {	/* /(..+:\s)/ is a reply indicator */
			if (*cpnew == ' ') {
				if (cpnew < cp + 3)
					break;		/* at least 3 char before ' ' */
				if (*(cpnew - 1) != ':')
					break;		/* require ':' before ' ' */
		...
		if (serial) {
			...
			if (i == serial) {	/* match before serial */
				...
				if (serial != j) {	/* got a '#' */
					 ...
					 * skip number/space 
					 */
					while (cpnew <= cpend && (*cpnew == ' ' || (*cpnew <= '9' && *cpnew >= '0')))
						++cpnew;
					i = serial + 1;
 ...
 */
static int
trimend(char *indata, unsigned int *np)
{
	char           *cplast;
	int             junk;
	int             r = 0;
	...
	while (junk) {
		junk = 0;
		while (cplast >= indata && (*cplast == ' ' || *cplast == '\t' || *cplast == '\r' || *cplast == '\n'))
			--cplast;
 ...
 */
int
unfoldHDR(char *indata, unsigned int n, stralloc *outdata, const char *charset, stralloc *prefix, int flagtrimsub)
{
	int             r = 0;
	...
	cpend = cp + n - 1;
	cpnext = cp;
	if (!stralloc_copys(&tmpdata, ""))
		die_nomem();
	if (!stralloc_ready(&tmpdata, n))
		die_nomem();
	...
	if (!case_diffb(charset, 11, "iso-2022-jp")) {
		...
		while (cp < cpend) {
			...
			if (*cp == ESC)		/* maybe another G0 designation */
				if (*(cp + 1) == '(' || *(cp + 1) == '$') {	/* yep! */
					if (!stralloc_catb(&tmpdata, cpnext, cpesc - cpnext))
						die_nomem();
					cpnext = cp;
					continue;
				}
		...
		if (!stralloc_catb(&tmpdata, cpnext, cpend - cpnext + 1))
			die_nomem();
		if (reg != 6) {			/* need to return to us-ascii at the end of the line */
			if (!stralloc_cats(&tmpdata, TOASCII))
				die_nomem();
	...
	if (!case_diffb(charset, 11, "iso-2022-cn") || !case_diffb(charset, 11, "iso-2022-kr")) {
		/*
		...
		while (++cp <= cpend) {
			if (*cp == SI || *cp == SO) {
				if (state == *cp) {	/* already in state. Skip shift seq */
					if (!stralloc_catb(&tmpdata, cpnext, cp - cpnext - 1))
						die_nomem();
					cpnext = cp;
				} else			/* set new state */
					state = *cp;
		...
		if (!stralloc_copyb(&tmpdata, cp, n))
			die_nomem();
	}
	 ...
	 */
	if (!stralloc_copys(outdata, ""))
		die_nomem();
	if (!stralloc_ready(outdata, n))
		die_nomem();
	outdata->len = n;
	cpout = outdata->s;
...
/*
 * $Log: unfoldHDR.c,v $
 * Revision 1.1  2025-01-22 11:01:06+05:30  Cprogrammer
 * Initial revision
 *
 */
