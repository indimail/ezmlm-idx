/*
 * $Id: ezmlm-list.c,v 1.1 2025-01-22 10:52:38+05:30 Cprogrammer Exp mbhangui $
 */
#include <unistd.h>
#include "strerr.h"
#include "substdio.h"
...
const char      USAGE[] = "ezmlm-list: usage: ezmlm-list [-mMnNvV] dir [subdir]";
...
static int
subwrite(const char *s, unsigned int l)
{
	return substdio_put(subfdout, s, l) | substdio_put(subfdout, "\n", 1);
}

static int
dummywrite(const char *s, unsigned int l)
{
	return (int) l;
	(void) s;
}
...
main(int argc, char **argv)
{
	...
	if (flagnumber) {
		...
		if (substdio_put(subfdout, strnum, fmt_ulong(strnum, n)) == -1)
			die_write();
