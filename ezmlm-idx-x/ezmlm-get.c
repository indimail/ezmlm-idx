/*
 * $Id: ezmlm-get.c,v 1.1 2025-01-22 10:52:37+05:30 Cprogrammer Exp mbhangui $
 */
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
static const char *digsz =
	"from\\to\\subject\\reply-to\\date\\message-id\\cc\\" "mime-version\\content-type\\content-transfer-encoding";
...
const char      USAGE[] = "ezmlm-get: usage: ezmlm-get [-bBcClLpPsSvV] [-f fmt] [digestcode]";
 ...
 */
void
write_ulong(unsigned long num, unsigned long cum, unsigned long dat, const char *fn, const char *fnn)
{
	int             fd;
 ...
 */
void
presub(unsigned long from, unsigned long to, stralloc *subject,
 ...
 */
void
postsub(int factype, /* action type (AC_THREAD, AC_GET, AC_DIGEST) */
		char format /* output format type (see idx.h) */)
{
	code_qputs("");
			...
			if (match) {
				if (flaginheader) {
					if (line.len == 1) {
						...
						if (constmap(&digheadersmap, line.s, byte_chr(line.s, line.len, ':'))) {
				...
				if (flaginheader) {
					if (line.len == 1) {
						...
						if (case_startb(line.s, line.len, "cc:")) {
							if (!stralloc_copy(&archcc, &line))
								die_nomem();
						} else
						if (case_startb(line.s, line.len, "date:")) {
							if (!stralloc_copy(&archdate, &line))
								die_nomem();
						} else
						if (case_startb(line.s, line.len, "from:")) {
							if (!stralloc_copy(&archfrom, &line))
								die_nomem();
						} else
						if (case_startb(line.s, line.len, "keywords:")) {
							if (!stralloc_copy(&archkeywords, &line))
								die_nomem();
						} else
						if (case_startb(line.s, line.len, "message-id:")) {
							if (!stralloc_copy(&archmessageid, &line))
								die_nomem();
						} else
						if (case_startb(line.s, line.len, "subject:")) {
							if (!stralloc_copy(&archsubject, &line))
								die_nomem();
						} else
						if (case_startb(line.s, line.len, "to:")) {
							if (!stralloc_copy(&archto, &line))
								die_nomem();
						}
					}
				} else
 ...
 */
void
msgout(unsigned long msg, char format)
{
	int             fd;
	unsigned int    len;

	if (!stralloc_copys(&filename, "archive/"))
		die_nomem();

	len = fmt_ulong(strnum, msg / 100);
	if (!stralloc_catb(&filename, strnum, len))
		die_nomem();
	if (!stralloc_cats(&filename, "/"))
		die_nomem();
 ...
 */
void
digest(msgentry *msgtable, subentry *subtable, authentry *authtable,
	unsigned long from, unsigned long to, stralloc *subj,
	int factype, char format)
{
			...
			for (msg = psubt->firstmsg; msg <= to; msg++) {
				if (pmsgt->subnum == subnum) {
					if (ffirstmsg) {
						...
						if (!stralloc_catb(&line, psubt->sub + HASHLEN + 1, psubt->sublen - HASHLEN - 1))
							die_nomem();
					} else
					if (!stralloc_copys(&line, ""))
						die_nomem();
					if (!stralloc_cats(&line, "\t"))
						die_nomem();
					if (!stralloc_catb(&line, strnum, fmt_ulong(strnum, msg)))
						die_nomem();
					...
					if (pmsgt->authnum) {
						...
						if (len > HASHLEN) {
							if (!stralloc_catb(&author, cp + HASHLEN + 1, len - HASHLEN - 1))
								die_nomem();
						} else
						if (!stralloc_catb(&author, cp, len))
							die_nomem();
						if (!stralloc_0(&author))
							die_nomem();
					}
					if (!stralloc_cats(&line, MSG1(TXT_BY, author.s)))
						die_nomem();
					if (!stralloc_append(&line, "\n"))
						die_nomem();
...
main(int argc, char **argv)
{
	...
	if (outformat == 0) {
		outformat = (getconf_line(&line, "digformat", 0)
	...
	if (!stralloc_copy(&listname, &outlocal))
	...
	if (act == AC_NONE) {
		...
		if (case_starts(action, ACTION_INDEX) || case_starts(action, ALT_INDEX))
			act = AC_INDEX;
		else
		if (case_starts(action, ACTION_THREAD) || case_starts(action, ALT_THREAD))
			act = AC_THREAD;
	}
	...
	if (act != AC_INDEX) { /* need to do header processing */
		if (!getconf(&digheaders, "digheaders", 0)) {
			if (!stralloc_copys(&digheaders, digsz))
				die_nomem();
			if (!stralloc_0(&digheaders))
				die_nomem();
			psz = digheaders.s;
			while (*psz) {
				if (*psz == '\\')
	...
	case AC_DIGEST:
		 ...
		 */
		if (!stralloc_copy(&line, &subject))
			die_nomem();
		if (!stralloc_0(&line))
			die_nomem();

		if (!stralloc_ready(&seed, HASHLEN + 1))
			die_nomem();
		seed.len = HASHLEN + 1;
		seed.s[HASHLEN] = '\0';
		...
		if (flagindexed && (outformat != NATIVE))
			idx_mkthreads(&msgtable, &subtable, &authtable, &datetable, mno, to, max, flaglocked);
		else
	...
	case AC_GET:
		 ...
		 * for rfc1153 
		 */
		if (!stralloc_copys(&subject, MSG1(SUB_DIGEST_OF, action)))
			die_nomem();
		...
		if (flagindexed && (outformat != NATIVE))	/* fake out threading */
			idx_mkthreads(&msgtable, &subtable, &authtable, &datetable, u, to, max, 0);
		else
	...
	case AC_INDEX:
		...
		doheaders();
		tosender();
		if (!stralloc_copys(&subject, MSG1(SUB_RESULT_OF, action)))
			die_nomem();
		...
		if (to < u)
			to = u;
		if (to > u + MAXINDEX)
			to = u + MAXINDEX;	/* max MAXINDEX index files */
		if (to > max)
			to = max;
		u /= 100;
		to /= 100;
		while (u <= to) {
			if (!stralloc_copys(&filename, "archive/"))
				die_nomem();
			if (!stralloc_catb(&filename, strnum, fmt_ulong(strnum, u)))
				die_nomem();
			if (!stralloc_cats(&filename, "/index"))
				die_nomem();
			if (!stralloc_0(&filename))
				die_nomem();
				...
				for (;;) {
					...
					if (match) {
						if (line.s[0] != '\t') {	/* subject line */
							...
							if (!stralloc_copyb(&line2, line.s, pos))
								die_nomem();
							if (!stralloc_catb(&line2, line.s + pos1, line.len - pos1))
								die_nomem();
						} else {
							pos = byte_chr(line.s, line.len, ';');
							if (pos + HASHLEN + 1 < line.len && pos > 15 && line.s[pos + 1] != ' ') {
								if (!stralloc_copyb(&line2, line.s, pos - 15))
									die_nomem();
								pos++;
								if (!stralloc_catb(&line2, line.s + pos + HASHLEN, line.len - pos - HASHLEN))
									die_nomem();
							} else /* old format - no author hash */ if (!stralloc_copyb(&line2, line.s, line.len))
								die_nomem();
						}
	...
	case AC_THREAD:
		...
		if (u == 0 || u > max) {
			...
			if (u + THREAD_AFTER > max) {
				...
				digest(msgtable, subtable, authtable, from, max, &subject, AC_THREAD, outformat);
			} else {
				idx_mkthread(&msgtable, &subtable, &authtable, from, u + THREAD_AFTER, u, max, 0);
				digest(msgtable, subtable, authtable, from, u + THREAD_AFTER, &subject, AC_THREAD, outformat);
			}
		}
		break;
	...
	default:
		...
		strerr_die2x(100, FATAL, "Program error: I'm supposed to deal with this but I didn't");
	}
	...
	if (act == AC_DIGEST) {
		if (chunk) {
			if (!stralloc_cats(&line, "-return-g-"))
				die_nomem();
		} else
	...
	if (act == AC_DIGEST) {		/* Do recipients */
		...
		if (chunk) {
			if (!stralloc_copys(&line, "T"))
				die_nomem();
			if (!stralloc_cat(&line, &outlocal))
				die_nomem();
			if (!stralloc_cats(&line, "-s-d-"))
				die_nomem();
			if (!stralloc_catb(&line, hashout, COOKIE))
				die_nomem();
			if (!stralloc_cats(&line, "-"))
				die_nomem();
			if (!stralloc_cats(&line, strnum))
			...
			if (!stralloc_cats(&line, "-"))
				die_nomem();
			if (!stralloc_copys(&line2, "@"))
				die_nomem();
			if (!stralloc_cat(&line2, &outhost))
				die_nomem();
			if (!stralloc_0(&line2))
				die_nomem();
			j = 0;
