...
int constmap_index(const struct constmap_idx *cm,const char *s,int len)
{
  constmap_hash h;
...
const char *constmap_get(struct constmap_idx *cm,unsigned int idx)
{
  if (idx <= 0 || idx > cm->num)
    return 0;
  else
...
const char *constmap_idx(struct constmap_idx *cm,const char *s,int len)
{
  constmap_hash h;
  struct constmap_entry *e;
  int pos;
...
int constmap_idx_init(struct constmap_idx *cm,const char *s,int len,int splitchar)
...
void constmap_idx_free(struct constmap_idx *cm)
{
